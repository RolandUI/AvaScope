...
internal static class Program
{
    ...
    private static async Task<int> Observe(string[] args)
    {
        const string usage = "Usage: avascope observe --request <observation.json> [--manifest-dir <dir>]";
        var options = ParseOptions(args, usage);
        if (!options.Success)
        {
        ...
        try
        {
            request = JsonSerializer.Deserialize<RuntimeObservationRequest>(File.ReadAllText(Path.GetFullPath(path!)), JsonOptions);
        }
        ...
        if (request is null)
        {
            WriteFailure<RuntimeObservationResponse>(InvalidCliArguments, "An observation request object is required.");
            return 2;
        }
