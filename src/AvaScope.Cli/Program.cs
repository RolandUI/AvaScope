...
internal static class Program
{
    ...
    private static async Task<int> RunScenario(string[] args)
    {
        ...
        if (!ValidateOptions(options.Values, GetRunScenarioUsage(), "request", "manifest-dir")
            || !TryReadRequiredOption(options.Values, "request", GetRunScenarioUsage(), out var requestPath))
        {
            return 2;
        }
    ...
    {
        return "Usage: avascope run-scenario --request <scenario.json> [--manifest-dir <dir>]";
    }
