...
internal sealed class LocalBridgeServer : IDisposable
{
    ...
    private async Task<BridgeIpcResponse> WindowAsync(BridgeIpcRequest request, CancellationToken cancellationToken)
    {
    ...
    {
        if (request.TextEdit is null)
            return BridgeIpcResponse.Fail(request.RequestId, new("text_edit_request_required", "A structured text edit request is required."));
        var result = await _runtime.EditTextAsync(request.TextEdit, cancellationToken);
        return result.Success ? BridgeIpcResponse.Ok(request.RequestId, result.Value)
            : BridgeIpcResponse.Fail(request.RequestId, ToProtocolError(result.Error!));
    }

    private async Task<BridgeIpcResponse> EnsureStateAsync(BridgeIpcRequest request, CancellationToken cancellationToken)
    {
        if (request.DesiredState is null)
            return BridgeIpcResponse.Fail(request.RequestId, new ProtocolError("missing_desired_state", "Desired-state options are required."));
        var result = await _runtime.EnsureStateAsync(request.DesiredState, cancellationToken);
        return result.Success ? BridgeIpcResponse.Ok(request.RequestId, result.Value)
            : BridgeIpcResponse.Fail(request.RequestId, ToProtocolError(result.Error!));
    }

    private async Task<BridgeIpcResponse> ExplainActionAsync(BridgeIpcRequest request, CancellationToken cancellationToken)
    {
        if (request.ActionExplanation is null)
            return BridgeIpcResponse.Fail(request.RequestId, new ProtocolError("missing_action_explanation", "Action explanation options are required."));
        var result = await _runtime.ExplainActionAsync(request.ActionExplanation, cancellationToken);
        return result.Success ? BridgeIpcResponse.Ok(request.RequestId, result.Value)
            : BridgeIpcResponse.Fail(request.RequestId, ToProtocolError(result.Error!));
    }

    private async Task<BridgeIpcResponse> ObserveChangesAsync(BridgeIpcRequest request, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(request.TopLevelId))
        {
            return BridgeIpcResponse.Fail(
                request.RequestId,
                new ProtocolError("missing_top_level_id", "Inspect node requests require a top-level id."));
        }
        ...
        if (string.IsNullOrWhiteSpace(request.TreeKind))
        {
            return BridgeIpcResponse.Fail(
                request.RequestId,
                new ProtocolError(BridgeErrorCodes.InvalidInspectRequest, "Inspect node requests require a tree kind."));
        }
        ...
        if (string.IsNullOrWhiteSpace(request.NodeId))
        {
            return BridgeIpcResponse.Fail(
                request.RequestId,
                new ProtocolError(BridgeErrorCodes.InvalidInspectRequest, "Inspect node requests require a node id."));
        }
