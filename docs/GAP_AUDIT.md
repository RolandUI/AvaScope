...
Selected next slice: `R0.4.0-M1 Bridge Session Discovery And Cleanup`. W18 completed CLI doctor/self-test, W19 completed preview profiles, W20 added a validated packaged-CLI agent workflow, W21 added deterministic runtime `clear_text` input, W22 added bounded diagnostics issue provenance, W23 reduced duplicate live-preview reloads with unchanged-input skip events, W24 added CI-friendly baseline-check reports, W25 completed the release-candidate audit and full local validation, and `v0.2.0` through `v0.3.0` shipped repeated-agent workflow hardening, PreviewHost diagnostic hardening, and deterministic animation sampling. Future work is planned release-by-release through `v1.0.0` in `docs/RELEASE_PLAN.md`; `v0.4.0` starts with runtime bridge discovery and cleanup because stale manifests, dead processes, duplicate records, and ambiguous attach undermine every later runtime workflow.
