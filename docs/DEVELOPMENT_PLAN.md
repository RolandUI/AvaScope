...
- `2026-09-25`: #194 is the sole active issue after a confirmed native Direct reset-order defect on clean afd4ad0: A then B, reset A, reset_all leaves A with zero active overrides instead of original Ada. The partial #174 mutation round retains 20 public calls (15 MCP/five CLI), six passing checks, one agent assertion mistake and one real failure; journal, five viewed images and unchanged source hashes are retained. Explicit fixture reset recovers; client 20728 exits 0 and owned app 21148 is terminated. Add failing real-Avalonia property/class/resource ordering regressions before a minimal guard, then native comparison. #174 ready; #193 review with CI 36107667148 still running on 511312a, #184 done. No release.
