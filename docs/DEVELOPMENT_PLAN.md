...
- `2026-09-23`: Release v1.5.0 / #156 covers #117-#153 and #155; #154 is cancelled (no foreign framework interoperability). #117-#139 are closed; #140-#145 await combined platform gates. #144 is in review at 8dc5bf5 and #145 at 9c797c4. The corrected full Linux suite passed 682 tests with 2 separate native-only skips; the preceding standalone-provider and real Windows/X11 gates passed. CI 35807347889 passed 681 tests but exposed an observation fixture sampling focus-driven layout before its first frame; that fixture now waits for its render fence. Active #146 adds bounded dispatch/operation/app diagnostic traces, workflow input correlation, redaction before retention, source availability and policy-owned exports. Windows and final Linux trace/operation/observation/lease/stable-contract checkpoints passed 21/21; the final Linux build had zero warnings/errors and includes the operation-ancestor exclusion regression. A fresh combined hosted CI is required before closing the review issues. Earlier unawaited headless-test results are not release evidence. Avalonia runtime/sample 12.1.3, Bridge standalone baseline 12.1.0; product version stays 1.4.2 until the validated release commit.
...
- Goal: bounded correlated dispatch/application diagnostics, sanitized before retention and export, with explicit unavailable sources.

## Next Action

Validate #140/#141 through corrected full and native gates, then implement #146-#153 and #155 in issue order before the #156 release gate.
