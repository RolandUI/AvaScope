...
- `2026-09-02`: Implemented #103 locally. Semantic workflows and runtime scenarios can declare workflow-scoped top-level aliases with exact title, kind, active-state, and optional session selectors; every aliased step, typed wait, assertion, screenshot, and automatic evidence capture re-resolves the active top-level immediately before use. Closed and reopened windows may receive new runtime ids without invalidating the workflow, while unknown, missing, ambiguous, and cross-session aliases return bounded candidates and remediation details. Results and scenario timelines retain the logical alias plus the resolved runtime id as diagnostic evidence. Protocol, Core, CLI, MCP schema/capability, documentation, idempotent replay, and real two-window headless coverage are included. Debug and Release builds passed with `0` warnings/errors and all `448` tests passed in both configurations; hosted validation remains pending.
...
- Goal: route one workflow across recreated Avalonia top-levels through stable semantic aliases.

## Next Action

Complete Release and hosted cross-platform validation for workflow-scoped top-level aliases, then close #103 and start #104.
