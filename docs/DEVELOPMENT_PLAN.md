...
- `2026-09-24`: #171 is the sole active investigation; #166 is in review. CI `36025169889` attempt 1 fails one full-suite Unicode range edit at the five-second IPC boundary (801 passed, 1 failed, five explicit native skips); all six native lab combinations pass. Original job log is retained. Fifteen isolated fresh-process repetitions on unchanged `2f3894f` all pass. Cause is not established, so no production timeout/dispatcher change or unconditional edit retry is justified. The failed hosted job and dependent gates are rerunning on the same source to characterize the failure. The prior native/preview exploration remains valid, but the combined campaign is not yet clean.
