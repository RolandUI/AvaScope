...
- Goal: add bounded observe-act-verify actions and deterministic automatic runtime failure evidence packs with aligned JSON, Markdown, and JUnit reports.

## Next Action

Add pre-execution validation and bounded expansion for conditional, retryable, optional, variable-driven, and reusable fragment execution while preserving idempotency, cancellation, alias routing, and deterministic evidence.
