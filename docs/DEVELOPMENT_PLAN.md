...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117-#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117-#135 are closed; CI 35777686856 passed Windows/Linux/native macOS, including #135 onboarding and CLI/MCP recipe conformance. Active #136 adds bounded action-blocker evidence, app-declared activation points, clipping/hit-test checks, dispatch revalidation (including focus callbacks), and geometry-revision rebinding for exported workflows. Release builds pass with 0 warnings/errors; 40 focused Windows and 27 Linux tests pass. The earlier full Windows run passed 592 tests with one opt-in native skip; its two stale surface expectations were corrected, and all six timing/file-lock failures passed in the focused rerun. Three-platform CI will validate the committed slice before closure. Product version stays `1.4.2` until the final validated release commit.
