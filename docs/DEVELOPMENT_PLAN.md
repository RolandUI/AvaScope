...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117–#153 and #155 (38 issues); #154 cancelled: no Robot Framework/PlatynUI interoperability, adapters or foreign workflow import. #117–#126 are closed after hosted Windows/Linux/macOS gates 35738762089 and 35745338184. #127 is implemented in 61b4752 and in review: Release build 0 warnings/errors, 90/90 focused tests, all 12 external-provider CLI/MCP success/failure runs passed on Headless/Win32/managed X11; macOS readiness joins the next combined gate. Active #128 now implements bounded coordinated observations: one bridge request for windows/focus/flat visual fragments/actions/validation and optional in-memory PNG capture, sampled change detection, partial failures, redaction before truncation, exclusions and Core masking before disk writes. The focused observation/policy/budget/protocol/MCP contract suite passed 112/112 with no build warnings/errors; real CLI/MCP parity and reduced payload are covered. Product version stays `1.4.2` until the final validated `Release 1.5.0` commit.
