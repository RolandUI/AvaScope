...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117-#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117-#136 are closed. #137 is in review at `49e4d7e`; CI 35785464651 passed Windows/Linux and awaits native macOS packaging. #138 adds `ensure_state` / `ensure-state` for checked/expanded/text/value/selection with public provider or routed input paths, before/after verification, generation/state guards, policy/provenance, bounded exact-request replay and truthful partial/uncertain outcomes. Release builds pass with 0 warnings/errors; the full Windows checkpoint passed 617 tests with one opt-in native skip, and final focused batches passed 103 tests each on Windows/Linux. Real Windows and Linux X11 CLI desired text, MCP replay and no-op checks passed in the native input/dialog gate. Headless cleanup now reuses the existing explicit-cleanup helper. #138 awaits committed CI; #139 form inspection/fill is next. Product version stays `1.4.2` until the final validated release commit.
