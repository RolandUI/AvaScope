...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117-#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117-#136 are closed; CI 35781923571 passed Windows/Linux/native macOS for #136. #137 adds bounded parent/ancestor/descendant/explicit-label selectors, typed projections, coverage/candidate evidence, and generation/recycling checks shared by find, workflow validation, waits, guarded inspection and dispatch. Release builds pass with 0 warnings/errors; 123 focused Windows and 43 Linux tests pass. The intermediate full Windows run passed 606 tests with one opt-in native skip; its two timing/cleanup failures passed in the focused rerun with bounded xUnit parallelism. #137 awaits committed three-platform CI; #138 is next. Product version stays `1.4.2` until the final validated release commit.
