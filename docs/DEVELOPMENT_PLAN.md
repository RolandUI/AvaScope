...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117-#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117-#133 are closed. Native macOS input run 35769751885 passed at a2c010a; #133 CI 35769024024 passed Windows/Linux and macOS full tests/runtime, with its later picker failure resolved by that dedicated native run. Active #134 implements AvaScope-native workflow export: matching execution fingerprints, actual verified conditions, stable-target review, parameterization and fresh-session policy bindings through the existing compiler/runner. The checkpoint passed 591 Windows Release tests (one separately gated native test skipped) and 123 focused Linux tests; five final export tests also cover missing window identity, private reports and required authorization. Cross-platform CI is the remaining export checkpoint. Product version stays `1.4.2` until the final validated release commit.
