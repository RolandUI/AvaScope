...
- `2026-09-26`: #226 is review; #174 resumes sole active. Exact targeted f309708 CI36253239362 succeeds:7 Linux readiness tests, four positive native-Wayland CLI/MCP1x/2x cases and four expected negative cases (text mismatch on both adapters, wrong backend, missing prerequisites). Seven owned app/compositor environments close and remove private runtime directories;500x460/1000x920 images match the two reviewed images byte-for-byte. Independent ci-wayland-f309708-verified-summary.json and reviewed-image-identity.json retained. Original226 timeout did not recur; original cause remains unknown and complete hosted validation of226 is pending. Full4b47791 CI36252124008 has three native labs passed, Windows artifact verification still running; preserve it, then dispatch a fresh current-head full gate. Next174 work: actual native runtime diagnostics, artifact/viewer boundaries and source/schema coverage reconciliation. Inventory61/43closed; no release/Retina claim.

- `2026-09-26`: First targeted Wayland run36252835811 on56ff03d is terminal failed in the new caller boundary: all four positive native CLI/MCP1x/2x cases and Linux7 readiness tests pass, then inherited PSNativeCommandUseErrorActionPreference=true throws on the intentional CLI negative exit before its response is persisted. This is not recurrence of226's capture timeout. Full artifact/log retained at ci-wayland-56ff03d-native; negative wrapper response and later guards are missing. A fresh pwsh -NoProfile -File invocation isolates the script's explicit negative-exit assertions and returns success only when they pass. Local real-child probe D:/AvaScope-QA-active/wayland-invocation-226/verification.json reproduces the original lost response, validates expected negative persistence/exit0, and preserves genuine assertion failure/exit1. Workflow-only caller correction and targeted rerun follow; original Wayland cause remains open. Preserve live full CI36252124008; no release.

- `2026-09-26`: #226's diagnostic slice passes7/7 local readiness tests (final15s, zero skips/build warnings/errors). Before: three controlled dispatcher/composition deadlines emit indistinguishable timeoutMs-only errors; four existing controls pass. After: bounded phase, consistent monotonic total/phase elapsed time, and public Processed/Rendered task states distinguish all three boundaries; each read-only probe recovers after UI progress resumes. Capture/observation code and deadlines/caller-cancellation filter remain unchanged. Proof D:/AvaScope-QA-active/readiness-phase-226/verified-regression-final.json; initial seven-pass candidate retained separately. Existing native-input workflow adds opt-in wayland_only=true pinned toUbuntu24.04, existing managed-Wayland script/prerequisites, focused readiness TRX and native evidence retention; default route unchanged. Local YAML parser absent, so hosted workflow acceptance/execution remains required. Commit/push and targeted hosted Wayland measurement follow; #226 stays sole active and original native cause is still unknown. Preserve full4b47791 CI36252124008. No release.

- `2026-09-26`: #224 follow-up4b477914acbaecdc6ecc246ed652a5586d886094 is pushed/review; exact fresh full CI36252124008 runs, including225. Preserve without duplicate/cancellation. #226 is now sole active: distinguish readiness frame-dispatch/composition/snapshot-dispatch deadlines before asserting a Wayland rendering fix. Original successful edit/failed screenshot and owned cleanup remain retained; local WSL unavailable. Use controlled queued UI/frame tests and bounded payload-free phase/task timing evidence, then the existing actual hosted Wayland lane. No timeout increase, fallback screenshot or edit replay; original native cause unproven. Inventory61/43closed; no release.

- `2026-09-26`: #224's follow-up now passes17/17 fixture tests (22s, zero skips; build zero warnings/errors). A controlled0.98 Fluent pressed-scale completion during frame preparation reproduces the macOS equality boundary: four controls pass, two controlled cases fail before. After: the pre-render forced-arrangement containment and unchanged local layout checks remain; full current transformed extents, actual hit point and mouse Reset work across both cycles/sizes. Core DLL is byte-identical before/after. Proof D:/AvaScope-QA-active/reset-transform-224/verified-regression.json. Public12.1.3 Fluent Button sources confirm75ms RenderTransform transitions; exact hosted animation timing remains unproven. Commit/push and one fresh combined full gate follow; #225 review, #226 is next. Local WSL is not installed, so actual Wayland validation uses the existing hosted Linux lane. Inventory61/43closed; no release.

- `2026-09-26`: #225 is pushed/review at995046f after17/17 controlled tests and native integration-startup-225-01: eleven public calls,944 checks,399 hashes, seven native/three rendered images reviewed. Unchanged066192b hosts/provider/client isolate new verifier tools. Disabled CLI/MCP observation/cleanup and early-exit/negative guards pass; all25 per-case owned process observations absent (23 distinct reused PIDs). Three enabled routes pass; Direct MCP reaches tree/inspect but screenshot IPC read times out5023ms/5000ms/zero bytes, retained separately under #182. Ten expected outcomes/one finding, not blanket pass. Proof D:/AvaScope-QA-active/integration-startup-225-01/verified-charter.json. Exact71fc60a CI36247707296 is terminal failed: Windows1121/eight skips; macOS1116/two #224 origin-equality failures/ten skips; Linux #226 Wayland1000ms capture failure; nine native/expiry1x runs pass. Independent ci-71fc60a-verified-summary.json retained. #224 resumes sole active for frame-changing Fluent pressed transforms before the next combined full gate; #226 ready. Inventory61/43closed; original timing causes/Retina remain open, no release.

- `2026-09-26`: #225's candidate passes17/17 integration-verifier tests (23s, zero skips; both builds zero warnings/errors). Before: four output fault/timeout cases propagate exceptions and leave the stopped own PID's manifest, while two controls pass. After: cleanup failures have their own stage and bounded process/output/recovery evidence; earlier observation/caller cancellation remains separate, own manifests recover, foreign manifests remain byte-identical. Five-second exit/output budget and one-MiB capture loop unchanged. Independent proof D:/AvaScope-QA-active/integration-cleanup-225/verified-regression.json; all13 controlled owned processes absent. Commit/push precedes fresh native comparison using the prior pinned app/provider binaries to hold the host constant. #225 stays sole active. Current71fc60a CI36247707296 has Windows and three native labs successful, Linux failed at managed Wayland screenshot readiness1000ms after a verified edit (#226 ready); macOS Runtime pending. Original Wayland/cleanup scheduling causes remain unproven. Inventory61/43closed; no release.

- `2026-09-26`: #225 is now sole active after #174's completed native integration-startup-174-01 charter on clean066192b production binaries: eleven real calls (six MCP/five CLI),947 independent checks,399 binary hashes, nine expected outcomes and two findings. Existing #182 recurs at the first visual_tree read (5107ms/5000ms/zero bytes); new #225 mislabels a disabled-host cleanup timeout as startup cancellation after a passed15000ms observation. Five native/three rendered images and three externalized tree artifacts reviewed; all25 owned app/client/server processes absent. Native observation gaps and verifier corrections are retained in the charter; no blanket pass. Evidence D:/AvaScope-QA-active/integration-startup-174-01/verified-charter.json. #224 is pushed/review at71fc60a5c0807f7c177cdb50220e5ee6165e5de4; exact fresh full CI36247707296 still runs, do not cancel/duplicate. Inventory60 defects/43closed. Next controlled cleanup/error-preservation regression for #225, then native retest and #182 diagnosis. No release.

- `2026-09-26`: #224's rendered-scene regression passes all15 AgentQaFixtureTests (24s, zero skips; zero build warnings/errors). Before: both original controls pass; both controlled hidden-frame cases hit Panel instead of Reset. After: a real current frame precedes hit testing, forced-arrangement containment is still checked before rendering, all geometry/pixel dimensions remain equal and actual headless mouse clicks reset the page in both cycles/sizes. Core assembly unchanged. Independent source/assembly/TRX proof: D:/AvaScope-QA-active/reset-render-224/verified-regression.json. Initial incorrect resize precondition is retained separately, not a product reproduction; original hosted scheduling remains unproven. Exact prior de20cfb gate retains1118/one/eight plus all105 #221–#223 tests passing and nine native/expiry1x runs; Windows Direct/standalone cycle2 table images reviewed inside the client. Commit/push and a fresh full gate follow, then resume #174's prepared native integration charter. No release.

- `2026-09-26`: #224 is sole active for the headless Reset hit test after an uncommitted layout; #174 review. Exact de20cfb full CI 36245199135 is terminal failed: Windows1118/one failure/eight skips, all three native labs successful, downstream macOS Runtime/Linux Installer skipped. Original containment assertion passed, so no native #177 recurrence is established. Controlled previous-frame regression and render-boundary investigation precede any correction. Eleven native integration startup cases are prepared against clean066192b with399 binary hashes; none executed yet. Inventory59 defects/43closed. No release.

- `2026-09-26`: #223 is pushed/review at de20cfbdaf354f4986597de5d07ff517f2b3c82f after independently verified 64/64 local tests; #221/#222 also review. Fresh combined full CI 36245199135 runs on that exact commit, including all three fixes. Preserve it without duplicate dispatch/cancellation. #174 resumes as sole active for actual integration startup, runtime diagnostics, artifact/viewer boundaries and capability reconciliation. Prior 4ffd1a2 full gate is verified successful; #192/#220 closed, inventory58/43. Original macOS timing and Retina gaps remain explicit; no release.

- `2026-09-26`: #223's diagnostic correction passes all 64 LocalBridgeClientTests (3m44s, zero skips; final build zero warnings/errors). Actual early screenshot failures previously became 30.56/30.12-second responder timeouts despite zero/one completed mutation; they now preserve the original capture assertion in 178.56/254.79ms, cancel/observe the owned responder and never repeat the edit. All 13 identity variants and simultaneous capture/responder failure preservation pass. At most 64 payload-free responder phase records retain UTC/elapsed/index/method/result; unchanged 30-second total deadline. Source/assembly/TRX/phase verification: D:/AvaScope-QA-active/mutation-ipc-223/verified-regression.json. Original macOS scheduling/listener cause remains unproven. Exact prior 4ffd1a2 CI 36241933958 succeeds all six jobs, independently verified Windows1083/eight skips, macOS1080/ten, Linux33, native13/zero and nine native/expiry1x runs; #192/#220 closed with their controlled boundaries and original evidence retained. Inventory58/43closed. #223 remains sole active through commit/push and new combined #221–#223 full gate; no release.

- `2026-09-26`: #222 is pushed/review at 00cad9fd658e1cb5fc0dade7b6c1819ebafaad49 with 41/41 focused tests and six independently verified public calls. #223 is now sole active: retain the original macOS five-request timeout, reproduce how an early capture failure is hidden by awaiting the responder first, and add bounded phase/result evidence plus owned responder cleanup. Original listener/scheduling cause remains unproven; no deadline widening or retry. Prior 4ffd1a2 full CI 36241933958 remains live (five jobs successful, macOS Runtime pending); it excludes #221/#222. No release.

- `2026-09-26`: #222 is sole active; #221 is pushed/review at 3509f50dbe3362495bf5ce77535bf2d833f9fbc2. Declaration recognition now excludes method/local-function bodies and bodyless return-type headers without a compiler dependency. First regression 21 failures/18 controls becomes 39/39; two additional attribute/directive cases fail before the final header correction, then all 41 pass (1s, zero skips/build warnings). Six actual CLI/MCP calls in integration-declaration-after-01 pass: declaration-only guidance, both genuine existing call locations unchanged, and #221 required-review precedence preserved. Source/assembly/TRX hashes, unchanged fixtures and nine exited owned processes independently verify in D:/AvaScope-QA-active/integration-declaration-222/verified-regression.json. A verifier assumption about the second real standalone call was corrected from direct source inspection; responses were unchanged. Fresh full CI for this source is pending; preserve live prior 4ffd1a2 run 36241933958 (Windows 1083/eight skips downloaded; four jobs successful). #223 remains ready. Inventory 58 defects/41 closed; no release.

- `2026-09-26`: #221 changes only status precedence. Correct before selection: nine failures/eight controls (777ms); after: 17/17, no skips (1s), zero build warnings/errors. Five fresh public calls preserve supported integration and change undeclared-framework CLI/MCP results to needs_review with identical existing locations/diagnostics; #222 declaration-only result remains open. Source/binary/TRX/response verification: D:/AvaScope-QA-active/integration-review-221/verified-regression.json; all owned clients exited. The premature eight-case stale-binary selection is retained separately and invalid. #221 commit/review follows. Exact cb7ca45 CI 36239596341 fails only macOS LocalBridgeClientTests mutation-evidence responder's 30.094s wait for five IPC requests (#223 ready); downloaded Windows 1077/eight skips, macOS 1073/one fail/ten skips, Linux 33, native 13/zero skips and nine lab runs verify. New required CI 36241933958 runs on 4ffd1a2, including #192 but excluding #221. Inventory 58 defects/41 closed; no release.

- `2026-09-26`: #221 is sole active after the clean 4ffd1a2 integration/diagnostic charter finds two static-guidance defects through both CLI and actual MCP: required framework review is hidden by already_integrated (#221), and an uncalled loader declaration suppresses guidance (#222, ready). Fifteen calls/nine MCP/six CLI and 199 independent checks retain 11 passing controls/four failing observations, 35 provider hashes, source preservation and all owned clients/platform-probe exits. No app was launched. Regression-first status precedence is next; #174 review. #220 exact cb7ca45 gate has Windows 1077/eight skips, native input13/zero skips and all nine native lab runs independently verified; macOS Runtime still runs, Linux Installer passed. Do not cancel it; next full gate includes #192 and current completed fixes. Inventory57 defects/41closed; no release.

- `2026-09-26`: #192 is pushed/review at db6ac1407e9e4c39c4c7156879c6286550d89492 after the verified 14-case local gate and public schema control. #174 resumes as the sole active issue for explicit integration/doctor/profile/provider, runtime diagnostic and artifact/viewer boundaries. The current public schema still contains 75 tools; contracts and existing coverage are being reconciled before the next owned fixture journey. #220's exact cb7ca45 CI 36239596341 still runs (macOS native lab passed at the last check); after it terminates, dispatch a fresh full gate on the then-pushed db6ac14-or-later head. Do not cancel/duplicate that run. Inventory 55 defects/41 closed; original timing causes and Retina remain explicit. No release.

- `2026-09-26`: #192 diagnostic slice passes locally: original client fails the missing-trace regression after a successful health/exit; first candidate 12/12, final 14/14 (3m12s, zero skips), plus a real 75-tool schema response/control. Four controlled boundaries independently distinguish zero edits before startup from one actual edit while request outcome/output/shutdown is pending; no redispatch. 384 content-free phase records and selected source/assembly hashes verify in D:/AvaScope-QA-active/mcp-lifecycle-192/verified-regression.json. Client/test builds zero warnings/errors; existing 20/30-second waits and MCP shutdown budget unchanged. Original contention cause remains unproven. Commit/push and review handoff follow; fresh independent #192 CI must wait until separate cb7ca45/#220 CI 36239596341 terminates, to avoid canceling it. #199 closed, inventory 55 defects/41 closed; no native Retina or release claim.

- `2026-09-26`: #192 is the sole active issue for bounded MCP startup/request/exit evidence and controlled no-dispatch versus completed-edit diagnostics. #220 is pushed/review at cb7ca4597a1294cd346f8cc060383cb7ddb34b79; fresh exact full CI 36239596341 runs. #199's exact 125b346 full CI 36237048712 succeeds all six jobs: independently verified Windows 1076/eight skips, macOS 1074/nine, Linux 33, twelve native input cases/zero skips and nine owned native/expiry runs at 1x. Two hosted root delays correlate within a successful 14-node audit; short query deadline and cleanup pass. Its supported initial-connection boundary is reproduced/corrected; original machine conditions and native Retina remain unproven. Evidence: expanded-campaign/ci-125b346-windows-verified.json. No release.

- `2026-09-26`: #220's actual native capture method now has an isolated regression in the existing LifecycleTestApp: before-02 fails closed-stream/real-EOF completion after 3042ms with occupied workers; Windows dedicated readers pass in 0.733ms and still wait for a genuinely open writer. Nine native/accessibility/cleanup cases pass (64s, zero skips), plus nine existing lifecycle-verifier controls (11s); builds zero warnings/errors. Independent retained source/assembly/provider/TRX checks: D:/AvaScope-QA-active/native-eof-220/verified-regression.json. The initial harness write-before-reader deadlock is explicitly invalid and retained separately. Original native scheduling/pipe state remains unproven; no cleanup deadline/error suppression changed. #220 sole active through commit/push and fresh full gate. Prior #199 exact 125b346 CI 36237048712 has Windows 1076/eight skips and all twelve native cases verified, including two correlated delays/14 nodes/short deadline; full macOS/Linux still run. No release.

- `2026-09-26`: #220 isolated scheduling probe on Windows/.NET 10.0.7 confirms a supported stream boundary: an exited owned child plus four blocked probe-only workers leaves ReadToEndAsync pending for 3027ms (two queued items); dedicated reader threads complete actual stdout/stderr under the same bound. Original native scheduling/pipe state remains unproven; no #220 capture implementation changed yet. Retained D:/AvaScope-QA-active/native-eof-220 prototype/build/result is the next regression input; reuse LifecycleTestApp for isolated production-path validation. #199 exact 125b346 full CI 36237048712 still runs; prior four issue closures and 55/40 inventory verified. No release.

- `2026-09-26`: #220 is sole active for the remaining exited-native-host stdout completion failure; original stream/task scheduling versus pipe ownership is not yet established. #199 correction 125b346a62b449c1ddb8db8615e40bb08eaeeb31 is pushed/review and exact full CI 36237048712 runs; do not duplicate/cancel. Prior exact c079de6 CI 36234263189 passes all six jobs, independently verified Windows 1072/eight skips, macOS 1070/nine, Linux 33, Windows native 12/zero skips and nine owned native/expiry 1x runs. All 21 name/policy/masking regressions pass on each full test host. #215/#216/#217/#219 acceptance completes and issues/board close at done/100. Defect inventory: 55 tracked, 40 closed, including five defect-specific CI issues; #199 review and #218's original cause remain explicit. No release.

- `2026-09-26`: #199's controlled initial UIA connection boundary is reproduced and corrected locally: original 250ms provider fails ElementFromHandle in 256.318ms while the real root response takes 756.958ms. The identical fixture/test with a separate connection budget (Windows default 2000ms, capped by query; property transactions remain 250ms) passes two measured delayed root responses, 14 native nodes, actual CLI/MCP mapping/privacy/foreign guards and the 250ms outer-deadline control. Native test 1/1 (38s), focused 68/68 (40s), zero build warnings/errors. Independent binary/provider/timestamp verification is retained in D:/AvaScope-QA-active/uia-timeout-199/verified-burst.json. Continuous-delay candidate probe remains an honest outer timeout; original CI machine conditions remain unproven. #220 ready tracks the separately retained exited-host stdout drain failure, with #200's diagnostic preservation working. #199 sole active until commit/push and fresh gate; no release.

- `2026-09-26`: #199 is sole active to investigate the retained ElementFromHandle UIA_E_TIMEOUT recurrence with measured native readiness/budget evidence. #219 fix c079de6 is review: fresh complementary MCP-select/CLI-confirm manual Save flow passes, eight public calls, 72 independent checks, six viewed native images and one actual native click; app 4344 and all eight clients absent. Selection-stage UIA unavailable is explicit; reviewed dialog pixels plus final native UIA/public state agree on manual-save-árvíz.txt. Exact c079de61cfbd303f69350aedcac4fad952da9a08 full CI 36234263189 runs; do not duplicate/cancel it. #174 remains review, 54 defects/36 closed, no release.

- `2026-09-26`: #219 candidate replaces Win32 filename WM_SETTEXT with EM_SETSEL/EM_REPLACESEL under one finite send deadline. The real CLI-select/MCP-confirm regression fails before with the original suggestion and passes after with selected-save-árvíz.txt (77s); three prepared-picker controls pass (861ms), both builds have zero warnings/errors. Forty-four independent evidence/provider identity checks pass. The first local manifest-discovery failure remains unexplained; a later stale-provider run is an agent configuration error, both retained. Corrected run pins the actual host provider/Core; manual native complementary retest and fresh full CI follow. #219 sole active, no release.

- `2026-09-26`: #219 is sole active for the confirmed Windows Save picker mismatch: selected/confirmed synthetic-output.txt returns selected:native-save.txt to the app. #174 review; native-picker-standalone-02 retains 24 public calls (18 MCP/six CLI), 113 independent checks, twelve viewed native images, five native clicks and verified app/all-client cleanup. Open/cancel/one-shot/expiry controls pass; Save charter fails. Regression first, then evidence-backed Windows correction. Exact b377fb5 full CI 36230336282 failed at existing #199 with native ElementFromHandle HRESULT 0x80131505, no full query expiry; all three native lab jobs pass and downstream full Linux/macOS are skipped. Inventory 54 defects/36 closed. No release.

- `2026-09-26`: #174 is sole active; #218 diagnostic hardening is review at b377fb5 with original macOS cause unknown. Exact combined full CI 36230336282 runs on b377fb52060eae687d3ba2e8ce2b96d080891388; do not duplicate/cancel. Fresh inspection-standalone-01 completes 46 public calls (30 MCP/16 CLI), 276 independent checks, sixteen viewed native/two rendered images, eight native clicks and 31 pinned binaries. Logical trees, Unicode/paged changes, resync/expiry/privacy, visible highlights and explicit/frame cleanup verify; all 34 individual clients, persistent client 2092 and app 9560 are absent. Four observation-to-click attempts exceeded the five-second highlight lifetime, so this run does not prove input transparency while the overlay is active; original attempts and timings remain retained. No new product defect established. Continue remaining capabilities including native pickers and integration tooling; 53 defects/36 closed, no release.
...
- `2026-09-26`: #218 now preserves the complete failed workflow result and requires a genuinely matched missing observation on success; the original 200/100 ms deadlines and production logic remain unchanged. Fourteen relationship/readiness/protocol tests pass (32s), plus five real workflow/controlled expression-deadline tests (14s), zero skips/build warnings/errors. Original macOS failure and independent TRX hashes/counts remain in relationship-wait-01. This is diagnostic hardening, not a reproduced or fixed original cause. Commit/push and a combined full gate follow before returning to #174's pending standalone inspection comparison.

- `2026-09-26`: #217 scoped native proof completes on clean 69244a1: Direct/standalone child comparisons 16/15 public calls and 106/101 independent checks; each two native/three rendered viewed images. Real native incomplete main-tree scenarios add one public CLI/MCP call, 59 checks, one actual click and native/rendered image each; all pixels masked, truthful metadata, owned fallback and cleanup verified. 31 pinned binaries per selection; all owned apps/clients absent. #217 moves to review. Exact 2f7c686 full CI 36227569598 is terminal failed only on the macOS relationship disappearance wait (#218): Windows 1063/eight skips, macOS 1060/one failure/nine skips, Linux 33; fourteen #215/#216 regressions on both hosts and all nine native/expiry 1x runs pass. #218 sole active for retained diagnostics and measured investigation. Inventory 53 defects/36 closed. Stopped historical evidence is being SHA-256 verified into D:/AvaScope-QA-archive/20260926 with original paths preserved by junctions; current snapshots stay local. No release.

- `2026-09-26`: #217 candidate masks incomplete sensitive trees in full with truthful full_sensitive_mask metadata; selective complete-tree masks and outside-run refusal remain intact. Expanded before: two controls pass/eight pixel failures. After: all 104 affected tests pass (23s), zero skips/build warnings/errors; independent stage verifier retained. Fresh Direct host/provider are being built for native child-window comparison. #216 exact 2f7c686 full CI 36227569598 continues; no duplicate gate or release.

- `2026-09-26`: #217 sole active: real Skia policy regression confirms depth-truncated masking leaves an excluded deep control orange instead of black (one shallow pass/one deep pixel failure on 2f7c686). Original test-oracle inline/full-tree mistake retained separately. Conservative masking and depth/budget/public-surface regressions follow. #216 review: fresh native Direct/standalone each ten calls (six MCP/four CLI), 102 independent checks, two viewed images and 31 pinned binaries; typed excluded-name steps/reports and reset pass, documented outside-run tree artifacts fail closed with PNG removal. All twenty clients/apps absent. Exact 2f7c686 full CI 36227569598 runs; do not duplicate/cancel. 52 defects/36 closed; no release.

- `2026-09-26`: #216 cause confirmed: excluded/redacted automationNameStatus placeholders violate its optional enum and discard typed workflow evidence. Before: three controls pass/five failures, including the real workflow with retained runtime_evidence_redaction_failed diagnostics. Candidate withholds the optional status only when policy rewrites it, preserving names/key redaction and truthful ordinary states; all 221 affected tests pass in 3m04s, zero skips/build warnings/errors. Independent stage/CI verifier retained. Prior exact 1335a4c CI is terminal failed: Windows 1055/one/eight skips, all six name regressions and nine native/expiry runs pass; full macOS/Linux jobs skipped. Fresh #216 native Direct/standalone follows before a new combined full gate. #216 sole active; #174/#215 review. 51 defects/36 closed; no release.

- `2026-09-26`: #216 becomes sole active after #215's exact 1335a4c full CI 36225116901 fails Windows evidence-policy workflow (1055 passed/one failed/eight skips): expected failure step list is empty, original diagnostics discarded. Strict new accessibility-name availability versus excluded-subtree placeholders is the source hypothesis; preserve and reproduce before correcting. Native jobs continue; no duplicate gate. #174 review: fresh inspection-direct-01 on clean 7c0b33c completes 42 public calls (26 MCP/16 CLI), 250 independent checks, eleven viewed native/two rendered images, two actual native clicks and 31 binaries. Cursor changes/resync/expiry/privacy, logical trees and real highlight/input transparency/cleanup pass; all 35 owned clients/app 14648 absent. Original missing outputDirectory request and individual-client highlight timing limitation retained; persistent MCP verifies active pixels. Standalone comparison pending. 51 defects/36 closed; no release.

- `2026-09-26`: #214 closes after exact de733c7 full CI 36222349163 succeeds all six jobs. Independent downloaded evidence: Windows 1050/eight skips, macOS 1048/nine, Linux 33, all 27 audit regressions on both and nine native/expiry runs at 1x. #215 full CI 36225116901 runs on exact 1335a4cb949b6f419f6ac94d6ac20b7b40a66cd1 (production a8ba007); do not duplicate/cancel. #174 sole active: source inventory confirms 75 MCP tools/78 CLI commands; reconciliation finds 1657 explicit calls in 84 runs, covering 56 tool names/32 command names. Forty raw CLI payloads remain unclassified without explicit argv; these invocation counts are not coverage passes. Next fresh native boundary: logical inspection, observation cursors and highlight lifecycle through both transports. 50 defects/36 closed; no release.

- `2026-09-26`: #215 a8ba007 passes fresh native Direct/standalone: each twelve public calls (seven MCP/five CLI), 126 independent checks, four viewed images and 31 pinned binaries. All nine visual/logical tab peer names match actual Windows UIA; truly unnamed controls and intentional validation still report correctly. Raw declarations stay separate, audits leave journals unchanged, reset and owned apps 4628/17980/all 24 clients absent verify. One stale PowerShell LASTEXITCODE guard falsely reported Stop failure; structured termination and independent cleanup verify success without repeating Stop. #215 review pending its full gate; #174 resumes as sole active to reconcile all public capability evidence and remaining exploration. Prior #214 de733c7 CI 36222349163 has Windows/Linux/all native labs successful; final macOS packaging continues. 50 defects/35 closed; no release.

- `2026-09-26`: #215 candidate passes all 166 affected fixture/query/form/expression/virtual-item/native-accessibility/protocol/audit tests (85s, zero skips; build zero warnings/errors). Six real peer scenarios cover header updates, explicit/label/custom/empty/throwing names through visual/logical inspection, and both actual public audit transports. Retain original 6/8 run (six missing test expansion flags), corrected 7/7 baseline, first candidate 70 passes and 95/one compatibility failure. Final correction preserves declared automationName and adds effectiveAutomationName/automationNameStatus; actual peer failure is unavailable, not absent. Pinned Avalonia TextBlock peer ignores attached name, so old declaration contract remains unchanged. Independent verifier records all stages. Fresh native Direct/standalone verification follows; #215 sole active. #214 exact de733c7 CI 36222349163 has Windows/all native labs successful, macOS/Linux continue. 50 defects/35 closed; no release.

- `2026-09-26`: #214 de733c7 fresh native Direct/standalone each pass the scoped 25-call audit charter (16 MCP/nine CLI), 179 independent checks, four viewed images and 31 pinned binaries. Full/partial source versus report limits, real validation, scope absence/unavailability, reset and owned cleanup verify; all 50 clients/apps 16504/17444 are absent. Full de733c7 CI 36222349163 runs; #214 moves to review pending that gate. Separately #215 confirms nine false missing-name findings on ordinary TabItems in six public audits despite matching actual Windows UIA names; original evidence/verifier retained. #215 becomes sole active. Prior f2ab747 gate closed #209/#210/#211/#212; current inventory 50 defects/35 closed. No release.

- `2026-09-26`: #214 candidate now preserves requested-depth/source coverage, verifies bounded full tree evidence before auditing, resolves available deep scopes, distinguishes unavailable from absent, and reports real noninteractive validation errors. Original eight public CLI/MCP cases fail before and pass after; an additional real TextBlock error reproduces the actionable-only omission. Expanded run retains 109 passes/one obsolete depth8 exit assertion; corrected final affected set passes all 41 (35s), zero build warnings/errors. All stages independently verified in audit-coverage-01. Fresh native Direct/standalone validation is next; #214 remains sole active. Prior exact f2ab747 full CI 36219846271 succeeds: Windows 1021/eight skips, macOS 1019/nine, Linux 33 and all nine native/expiry runs verified at actual 1x. #209/#210/#211/#212 acceptance is complete; that gate excludes later #213 controls and this #214 candidate. No release.

- `2026-09-26`: #174 `audit-direct-01` on clean 69f6590 confirms #214: UI/design audits lose source coverage, report incomplete trees clean/full, and reject valid deep scopes on CLI/MCP. Twenty-one calls (14 MCP/seven CLI), 90 independent evidence checks, four viewed native images and 31 pinned binaries retained at Win32 1x; this is a failed audit boundary, not a passing charter. Full tree has 195 nodes versus 27 inline, actual validation/clipping agrees with pixels/journal/targeted inspection. Reset and owned app 15520/all 21 client absences verify. The old runtime-direct-01 path was correctly refused before launch; its original run was untouched. #174 returns to review; #214 becomes sole active for coverage/correct-scope regressions and repair. #213 69f6590 remains review/original cause unknown. Full f2ab747 CI 36219846271 continues. 49 defects/32 closed; no release.

- `2026-09-26`: #213 controlled queued/zero-viewport deadline tests and all affected fixture cases pass (23/23, 15s; zero build warnings/errors). Existing counters distinguish zero dispatched work from 26 scroll requests/5200 key reads, both with no selection or late selection after dispatcher drain. Original keyed-list assertion now retains cycle, actual list/table selection, container/list/client geometry and independent journal. Original #213 cause remains unknown because its counters were discarded; no product/deadline/replay change. #211 f2ab747 is pushed/review and exact full CI 36219846271 is running; do not duplicate/cancel. #213 sole active while recording this evidence; #212/#209/#210 review. 48 defects/32 closed; no release.

- `2026-09-26`: #211 controlled first-frame regression reproduces the retained macOS no-hit signature: arranged RTB pixels exist at (180,42), but no compositor hit target/hover exists until a real frame; after that, actual hover and cleanup pass. The common-state fixture now explicitly prepares/asserts that frame and target, preserving all six states, deadlines and reset assertions. Before: two controls passed; after: all 42 affected tests passed (42s), build zero warnings/errors. This establishes the local readiness boundary; hosted macOS confirmation and the combined full gate remain pending. Production code is unchanged in this slice; #211 sole active, #212/#209/#210 review, #213 ready/original cause unknown. 48 defects/32 closed; no release.

- `2026-09-26`: #212 native Direct/standalone on clean 1ebaaa1 each pass 29 corrected calls (17 MCP/12 CLI), 131/116 checks and five reviewed images at Win32 1x. Scoped identities/revisits versus truthful standalone uncertainty, context/revision/reset and stale/privacy/clear guards match journals/pixels. Thirty-one binaries per run and all owned app/client absences verify (65 clients including seven explicitly invalid initial Direct calls). Original depth-limit/helper outcome mistakes retained; corrected run uses depth32 and per-call outcome guards. #212 review/full gate pending; #211 resumes as sole active to investigate initial no-hit hover/frame readiness. #213 ready, cause unknown. GitHub inventory verifies 48 defects/32 closed, including four defect-specific CI tickets; no release.

- `2026-09-26`: #212 scoped navigation fixture passes all 26 affected tests after forced rebuild (19s, zero skips); first two runs retain 24/two and 23/three results. Two failures were an agent assertion error retained by an incremental build after editing during compilation; corrected binary is now verified. The additional keyed-list selection deadline failure is #213 ready, original cause unknown; assertions now retain full responses without changing deadlines. Fresh Direct/standalone hosts are being published for native verification. Previous exact 16a44ac CI failed macOS common-state first hover with no hit target; all other jobs and 21 #209/#210/#211 regressions pass. #212 sole active, #174/#209/#210/#211 review. 48 defects/32 closed; no release.

- `2026-09-26`: #212 is sole active; #174 review. Expand a scoped synthetic-document navigation fixture with two contexts, overview/details, per-document revisions/reset generation and Direct-only debug identity, while keeping the standalone source bridge-free. Validate real actions/journal/loop/privacy and native Direct/standalone journeys before claiming coverage. Full 16a44ac CI 36193600154 continues; downloaded Windows TRX verifies 1018 passed/eight skips plus all nine clean native/expiry runs at 1x. macOS/Linux artifacts pending. 47 defects/32 closed; no release.

- `2026-09-26`: #174 `windows-standalone-01` on clean b650ca1 (production 16a44ac) completes 46 calls (30 MCP/16 CLI), 126 window plus 58 navigation checks, nine viewed images. Geometry/modal/focus/closure and retrospective navigation boundaries pass; missing host identity leaves unique visits and explicitly uncertain sampled loop candidates. Initial verifier's zero-loop assumption is retained/corrected without replay. Reset, owned app 19560/all 46 client absences and 31 binaries/pure host/provider verify. Next fixture gap: explicit navigation surface/context/revision. Full 16a44ac CI 36193600154 has Windows and three native labs passed; Linux/macOS continue. #174 sole active; #209/#210/#211 review. 47 defects/32 closed; no release.

- `2026-09-25`: #211 16a44ac is pushed/review; full CI 36193600154 runs on exact 16a44acbc28ed745f501c4ebf4729ec08b23b143, do not duplicate/cancel. #174 resumes as sole active for a fresh standalone window/modal comparison and navigation boundaries. The new provider is packaged from unchanged candidate source; the pure standalone host is unchanged and will be independently hashed. Local Direct app/all clients remain absent. #209/#210 review and all original/transient causes remain explicit. 47 defects/32 closed; no release.

- `2026-09-25`: #211 diagnostic correction passes all 19 targeted cases (one minute), including eight new mapper and actual CLI/MCP occlusion/advisory controls. Original before retains five failures/three controls; first wider run retains 59 passed/five failures (two deep matrices, two geometry queries, one MCP cancellation). Added full-result assertions preserve future causes without changing deadlines or semantics; a 100% CPU snapshot is not causal proof. Original macOS common-state failure and these local transient causes remain unknown/open. Commit/review and one fresh combined full gate follow; then #174 standalone window/navigation exploration. No local native app or request client remains. 47 defects/32 closed; no release.

- `2026-09-25`: #174 `windows-direct-01` on clean 53ad53c passes 32 public calls (22 MCP/ten CLI), 124 retained checks and seven viewed native images: resize/stale/restore, owned child move, modal blocking/focus, isolated closure and closed-target refusal. Original W013 agent omission and verifier/cleanup invocation mistakes remain retained; corrected evidence verifies reset and all owned processes absent (one PID now belongs to svchost, never touched). Full 19dd8bd CI 36186815465 is terminal failed: Windows 1010/eight skips, Linux 33, all six native/three expiry runs pass; macOS 1007/one common-state matrix failure/nine skips, later macOS gates skipped. All thirteen #209/#210 cases pass on both, but tickets remain review until a complete gate. New #211 records the unexplained matrix failure and hidden diagnostics; it is now sole active, #174 review. Standalone window comparison and navigation remain pending. 47 defects/32 closed; no release.

- `2026-09-25`: #174 `focus-standalone-after-01` on clean 2684a38 passes 24 public calls (15 MCP/nine CLI), two actual Windows keyboard actions and 61 independent checks, with eight viewed images. Synthetic/native focus round-trips agree with app journals and independent UIA; the immediate native-Tab UIA lag is retained separately and converges on a fresh read without redispatch. Deep/shallow/missing recording, stale refusal and action-map boundaries pass. Twenty-five pinned binaries/source verify; owned app 5680/all original clients are gone (initial PID-only audit collision retained, final start-time-aware audit passes). Full 19dd8bd CI 36186815465 continues; #209/#210 review. Next: multi-window/modal boundaries. #174 sole active, 46 defects/32 closed; no release.

- `2026-09-25`: #210 19dd8bd is pushed/review; full combined #209/#210 CI 36186815465 runs on exact 19dd8bd349f39a58a642e3a235923d966c4b0f09. All owned local apps/clients are gone. #174 resumes as the sole active issue for fresh standalone focus/probe/action-map and corrected recording exploration, using real state/pixels and guarded ownership. Prior 59b9b2c full gate remains successful; 46 defects/32 closed. No release.

- `2026-09-25`: #210 is ready for commit/review after all 75 applicable local cases pass (73 plus two consumers; two Unix skips separately). Fresh Direct/standalone each execute 17 public calls (12 MCP/five CLI), pass 153/155 checks plus eight exact pixel-extent comparisons, and retain eight reviewed images. Full transformed rectangles agree across find/inspect/recording/picking; explicit local metrics and missing-target refusal remain correct. Source/binary hashes and owned app/all-client absence verify. Original before failures, obsolete legacy expectation and one agent pixel-parser error remain retained. Next: one full combined #209/#210 CI, then #174 exploration. 46 defects/32 closed; no release.

- `2026-09-25`: #210 candidate passes 73 affected tests/two Unix skips plus both bounds/gesture consumers after correcting one legacy assertion that expected the original geometry discrepancy. Before baseline retains five failures/two normal controls; all seven transformed pixel cases now pass. Fresh Direct `transformed-direct-after-01` has 17 calls/153 checks, eight reviewed images and eight exact pixel-extent matches (pressed 122.5x31.36 DIP); local layout metrics, 1.15x ancestor scale, picking, missing refusal and owned cleanup verify. Standalone comparison starts with fresh verified provider 82844218f1...; full combined #209/#210 gate remains pending. #210 sole active, 46 defects/32 closed; no release.

- `2026-09-25`: #209 2fbbee7 is pushed/review; #210 is sole active for full transformed visual extents with real normal/translation/scale/rotation/ancestor/clipping pixel regressions. Full prior 59b9b2c CI 36177716925 succeeds all six jobs: Windows 997/eight skips, macOS 995/nine, Linux 33 and six native/three expiry checks verified at 1x. #206/#207 close; #208 diagnostic slice validates but original installer stall remains unknown/open. Next complete gate will combine #209/#210 after native validation. No owned local app/client remains. 46 defects/32 closed; no release.

- `2026-09-25`: #209 candidate is ready for commit/review: 37 affected tests; fresh Direct nine calls/116 checks and standalone nine/118, six viewed images each. Bounded deep target/parent resolution, missing refusal and hover/down/release work with source/binary identity and owned cleanup verified. #210 transformed extents remains explicitly failed on both transports/integrations and is next. #209 full gate remains required; earlier 59b9b2c CI has all jobs passed except ongoing macOS packaging. 46 defects/30 closed; no release.

- `2026-09-25`: #209 native Direct `recording-direct-after-01` retains nine calls/116 checks and six viewed images: deep target/parent geometry now resolves through CLI/MCP, missing targets fail, hover/pressed/release render and clear with unchanged action counters. App 21968/all nine clients are gone. Separate #210 records mixed transformed origins/unscaled dimensions exposed by the real Fluent pressed scale; this remains failed, not whole-geometry success. A verifier's stationary-origin assumption was corrected with original failure retained. Fresh standalone host/provider (manifest a45b0e1b...) are built and isolated startup is running. #209 remains sole active; #210 ready. Earlier 59b9b2c CI verifies Windows 997/eight skips and six native/three expiry checks at 1x; Linux/macOS full jobs continue. 46 defects/30 closed; no release.

- `2026-09-25`: #209 candidate passes 37 affected cases (39s), including six new real geometry/coordinate/clipping/missing/replaced and retained-pipe disappearance cases. Original before has two failures/two passes; first candidate has 33 passes/two failures because it wrongly reused a tree context for node inspection, corrected to pin the actual observed node. All stages retained/independently verified. Fresh native Direct publish/start begins; standalone comparison follows. Earlier CI 36177716925 continues; no full pass claimed. #209 sole active, 45 defects/30 closed; no release.

- `2026-09-25`: #174 focus-recording-direct-01 on clean 703c960 retains 22 calls (14 MCP/eight CLI), 45 independent checks and five viewed images. Focus round-trip/stale guards/action-map pass; deep recording fails on both transports while shallow geometry passes. New #209 is sole active for bounded deep recording geometry; #174 review. Owned app 9580/all clients are gone; 25 pinned binaries and unchanged production source verify. Earlier full CI 36177716925 continues unchanged. 45 defects/30 closed; no release.

- `2026-09-25`: #207 test-only provenance follow-up 59b9b2c is pushed/review, all 32 local cases pass. Previous 8d10546 CI 36174208805 is terminal failed at the retained old provenance expectation; all native jobs succeed, full downstream jobs skipped. Combined full CI 36177716925 targets exact 59b9b2ccaee44947ea99f3fc4827bd56723fa641 including #208; do not cancel/duplicate. #174 resumes as the sole active issue for actual native focus/action-map/interaction-recording exploration with independent state, images and owned cleanup. 44 defects/30 closed; no release.

- `2026-09-25`: #207 provenance follow-up passes all 32 real provenance/picking cases (2m45s). It observes one enter/exit, synthetic dispatched provenance for actual cleanup, and unchanged not_dispatched for a repeated outside no-op. No production change; original 8d10546 CI failure retained. Follow-up is ready for commit/review with #208 4a15d53 already pushed/review. Full CI 36174208805 native labs still need to finish before one fresh combined gate; do not cancel/duplicate. No owned local app/client/test process remains. 44 defects/30 closed; no release.

- `2026-09-25`: #208 diagnostic/cleanup slice 4a15d53 is pushed/review; original installer stall cause remains unknown. #207 is sole active again: retained 8d10546 CI fails the old first-outside-move provenance expectation; the regression now observes real enter/exit state/events and retains not_dispatched for a second no-op outside move. Focused provenance/picking validation is running, no pass claimed yet. CI 36174208805 native labs still run; do not cancel/duplicate. No owned local native app/client remains. 44 defects/30 closed, no release.

- `2026-09-25`: #208 candidate confirms the owned-child leak before (two pass/one failure, 62s; fallback cleanup verified) and passes all six discovered installer cases in both after/final runs (90s each, one packaged-artifact guard). Independent TRX verification confirms original cancellation, bounded output tails, actual termination and successful install/discovery/MCP calls; default 60s and installer behavior unchanged. Original cause remains unknown. Ready for commit/review. #207 8d10546 CI 36174208805 fails one stale provenance expectation (Windows 993/one/eight); all eight new hover cases and the original installer case pass. Native labs continue, downstream full jobs skip. Next return to #207 for a real-exit versus repeated-no-op provenance regression; do not cancel/duplicate the current gate. 44 defects/30 closed; no release.

- `2026-09-25`: #207 8d10546 is pushed/review; exact-source full CI 36174208805 targets 8d10546c2a04fef303a8eb6b8bc842a7e0b95077 and is queued/running, do not cancel/duplicate. All local native apps/clients are gone. #208 is now sole active for controlled installer child-process timeout/failure evidence and bounded ownership cleanup, preserving the original unknown cause and the default deadline. Read the retained d32e539 TRX/job log and existing cleanup patterns before implementation. #206 remains review until a complete gate passes. 44 defects/30 closed; no release.

- `2026-09-25`: #207 candidate is ready for commit/full CI: final 56 affected passes/two Unix skips; fresh native Direct twelve calls/76 checks and standalone twelve/78, six viewed images each. Both public transports/integrations show actual hover class/3859 changed pixels, pressed/disabled/reset, delayed hover/clear and negative target guards. Source/binary hashes and input journals verify; owned apps 8460/9204 and all clients are gone. Standalone pure host/provider da3987d8... verifies. #207 sole active until commit/review; #208 installer evidence/cleanup is next. #206 stays review after d32e539 gate fails its installer test, with its five new cases and all native/expiry runs passing. 44 defects/30 closed; no release.

- `2026-09-25`: #207 candidate's native Direct run `hover-direct-after-01` passes twelve public calls/76 independent checks and six viewed images: actual hover class/3859 changed pixels, pressed/disabled/reset, delayed hover and explicit clear, missing/stale refusal, unchanged source/binary hashes and journals. Owned app 8460/all twelve clients are gone. Fresh standalone host/provider are built (manifest da3987d8...), and `hover-standalone-after-01` startup is in progress. #207 remains sole active (first two acceptance items checked); no source changes since the final 56-pass/two-skip test run. Full #206 d32e539 CI is terminal failed at the installer test (#208 ready), with all five #206 cases and six native/three expiry checks independently verified; full macOS/Linux jobs skipped. 44 defects/30 closed, no release.

- `2026-09-25`: #207 remains sole active. Five real hover regressions fail before; public enter/exit ownership, capture continuity, leaf-to-root cleanup and observed pseudo-class validation pass 56 affected tests/two Unix skips (68s), including eight new state/pixel/cleanup cases. One intermediate new-test coordinate mistake (35 pass/one fail) is retained and corrected with actual translated geometry. Fresh native Direct/standalone validation is starting on d32e539 plus the candidate. Full #206 CI 36168286222 fails Windows (985 pass/one installer timeout/eight skips); all five #206 cases pass, downstream macOS/Linux gates skip, native labs continue. New #208 retains the original 60-second installer process wait and missing diagnostic/cleanup evidence, ready behind #207; cause unknown. 44 defects/30 closed, no release.

- `2026-09-25`: #205 closes after complete ba07220 CI 36163086325 succeeds: Windows 981/eight skips, macOS 982/seven, Linux 33, all twelve new binding-scope cases on both full platforms and six native/three expiry checks verified at 1x. #206 d32e539 is pushed/review; fresh full CI 36168286222 targets exact d32e539938489d636b76c41cb50fc29dab067664 and is running, do not cancel/duplicate. #207 is the sole active issue for the retained Direct/standalone false-hover result; verify public Avalonia 12.1.3 enter/exit versus move semantics and add real state/cleanup regressions before implementation. No owned local app/client remains. Fresh GitHub accounting: 43 defects/30 closed; no release.

- `2026-09-25`: #206 candidate native verification is complete: Direct eleven calls/73 checks and standalone eight/75, three viewed images each, fresh pure host/verified provider, unchanged candidate and snapshot hashes, correct pressed/disabled/restored state and missing/stale refusal. Both explicitly retain #207's false hover success. Owned apps 2972/10996 and all clients are gone. The focused 27-case run and final three deep/replacement cases are independently verified; #206 is ready for commit/review/full CI. Prior #205 ba07220 CI 36163086325 has Windows 981/eight skips (all twelve binding-scope cases), Linux 33 and six native/three expiry checks verified; macOS packaging continues, do not cancel/duplicate. 43 defects/29 closed, no release.

- `2026-09-25`: #206 bounded-target candidate passes 27 affected tests (31s), then all three final real deep-target/replacement cases (8s). Native Direct replay retains eleven calls/73 independent checks: correct deep-target evidence, visible pressed/disabled states, restoration and zero-effect missing/stale refusal. It also confirms new #207: both transports falsely report pointerover success with unchanged pixels/no pseudo-class; delayed inspection agrees. Three images viewed; owned app 2972/all clients cleaned up. #206 remains the sole active issue; standalone fresh host/provider are built and its isolated run is starting. #207 ready. Separate #205 ba07220 CI 36163086325 has Windows 981/eight skips and six native/three expiry checks verified; Linux/macOS downstream gates continue. 43 defects/29 closed; no release.

- `2026-09-25`: #174 pointer-focus-direct-01 discovers #206 on clean ba07220: public MCP/CLI pseudo-state requests find the deep enabled button, then fail because each state looks only in the inline tree budget. A shallow control passes. Ten public calls, 39 independent discovery/identity/cleanup checks and two viewed native images retained; one unsupported tab invocation and one ignored extra read argument are agent mistakes, not defects. Reset and owned Stop terminate app 7048; all ten clients are absent. #174 moves to review; #206 is the sole active issue. Two real Avalonia response-budget regressions fail before implementation (3s); bounded-target candidate validation is running. #205 full ba07220 CI 36163086325 continues. 42 defects/29 closed; no release.

- `2026-09-25`: #205 ba07220 is pushed/review, and full CI 36163086325 targets that exact source. Previous full 30d176a CI 36157963606 succeeds all six jobs: downloaded Windows 969/eight skips, macOS 970/seven, Linux 33, all twelve pointer cases on macOS (including both queued Unix controls), eleven #203 cases on both full platforms and six native/three expiry checks independently verified at 1x. #204 stays review because the historical preceding cause is unknown. #174 is the sole active issue; pointer-focus-direct-01 starts on clean ba07220 with newly published pinned Direct host, Win32 1x, owned app 7048. Actual native pointer/pseudo-state/recording/focus exploration is in progress. 41 defects/29 closed; no release.

- `2026-09-25`: #205 candidate maps public source locations after realization to effective runtime DataContexts; unknown compiled children get explicit informational provenance, and DataContext bindings use the visual parent. Final twelve pixel-backed scope cases pass (2m01s); preceding candidates also pass four existing typed/resource cases and 31 remaining smoke cases, with their exact-stage limits recorded. Original five failures and candidate realization/visual-parent failures remain retained. Six actual CLI/MCP calls pass 23 independent checks after the unknown-child correction; a seventh final MCP template check has exact literal-6 pixels and no warnings. All owned clients exited. #205 remains active until commit/gate. Separate 30d176a CI 36157963606 has Windows/Linux/all native labs successful; macOS packaging continues, do not cancel/duplicate. Full c83e50c gate closed #202; 41 defects/29 closed, no release.

- `2026-09-25`: #174 preview-followup-01 on 96c1656 completes sixteen public calls/eight per transport, 33 independent verification assertions and five viewed images; one agent argument error is preserved separately. Baseline equal/changed and JUnit outcomes, exact 28,734-pixel delta, watch change/no-change/closed guards and severity filtering agree. New #205 confirms false missing-context warnings despite correct local binding pixels on both CLI/MCP and a literal reference. All 16 clients are gone, session closed, source/binary hashes unchanged. #205 is next; #204 remains review. Separate c83e50c CI has Windows 968/six skips (all eleven #203 cases passed), Linux 33 and six native/three expiry checks verified at 1x; macOS packaging continues. 41 defects/28 closed; no release.

- `2026-09-25`: #204 candidate 96c1656 is pushed/review with 29 affected local passes/two Unix skips. #174 resumes as the sole active issue for isolated loose-XAML baseline comparisons, watch/reload and diagnostic filtering through public CLI/MCP with independent pixel/session/artifact verification. The separate c83e50c CI 36152235816 has passed Windows Build/Test/Pack and all three native lab jobs; macOS/Linux downstream checks continue. Downloaded evidence is being verified; no overall green gate or #204 Unix pass is claimed.

- `2026-09-25`: #204 test-only candidate preserves the pending Unix listener, cancels before disposing a failed response and retains bounded request/lifecycle traces plus the original exception. Controlled geometry failure first reproduces missing cancellation (nine pass/one fail); final local pointer group passes ten/two Unix skips in seven seconds, real Avalonia picking nineteen passes in 41 seconds. Two Unix queued-connection positive/negative controls await hosted validation; the original macOS cause is not inferred. Evidence independently verified under pointer-sequence-01. #204 remains active for commit/gate; separate c83e50c CI 36152235816 continues and must not be cancelled. 40 defects/28 closed; no release.

- `2026-09-25`: #203 diagnostic/cleanup slice c83e50c is committed/pushed and in review (29 affected passes; actual CLI/MCP timeout evidence and owned cleanup independently verified, original cause still unknown). Full CI 36152235816 targets that exact commit and is running; do not cancel/duplicate it. #204 is the sole active issue for the preserved macOS pointer fixture method mismatch and three-minute failure; begin with precise request/lifecycle evidence and controlled reproduction. Prior 8bfcec0 gate is failed, #202 remains review despite four passing cases on each full platform. #201 remains blocked on public clock control. 40 defects/28 closed, no local owned preview/test process, no release.

- `2026-09-25`: #203 candidate retains safe child/build progress and bounded owned cleanup; 29 affected cases pass (58 s). Actual MCP and final CLI baseline checks each time out both builds, retaining project_build phase, owned exit, safe output markers and honest failed reports. Final CLI observes restore markers at 43.740/40.403 s; original B004 cause remains unknown. All recorded owned host/build PIDs are absent; a later aggregate CPU snapshot is 100%, not a causal conclusion. Two agent fixture corrections remain retained. Full 8bfcec0 CI 36146649046 fails one macOS pointer request-sequence fixture (new #204 ready): Windows 957/six skips, macOS 955/seven/one fail, Linux 33, six native runs/three expiry checks verified. All four #202 cases pass both full platforms, but #202 remains review. #203 sole active, #201 clock API blocked; 40 defects/28 closed. No release.

- `2026-09-25`: #201 public timing audit pins official Avalonia 12.1.3 source and confirms inaccessible clock APIs with an actual package compile probe (four expected access errors). It is open/blocked on supported clock control; no workaround or fix is claimed. #203 is the sole active issue for bounded preview phase/owned-process evidence and controlled timeout validation, preserving the original B004 failure separately. #202 full CI 36146649046 continues; 39 defects/28 closed. No release.

- `2026-09-25`: #200 closes after full 108adc5 CI 36141416097 passes all six jobs: downloaded Windows 953/six skips, macOS 952/seven, Linux 33; seven cleanup regressions pass both full platforms, Windows native input twelve passed with complete owned cleanup, all six native lab runs/three expiry checks verified at 1x. #202 fix 8bfcec0 is pushed/review (24 affected tests passed, actual no-build creation verified); fresh full CI 36146649046 targets that exact commit and is running. #201 is the sole active issue for supported animation timing, with current 12.1.3 API evidence retained; no animation implementation yet. #203 retains the original preview timeout, #199 original cause remains open. 39 defects/28 closed; no local owned native/preview/test process, no release.

- `2026-09-25`: #202 candidate forwards baseline build selection flags and validates no-build. All four new regressions fail before; corrected affected run passes 24/24 (101 s); the first after run retains one agent fixture-directory mistake (23/one). Actual CLI no-build baseline creation succeeds for two viewed images. Subsequent normal-shell MCP check has one 60 s host timeout and one exact pixel match, retained as #203; restricted-launcher NuGet errors remain separate. #202 sole active pending commit/full CI, #201/#203 ready, #174 review; 39 unique defects/27 closed. Prior 108adc5 CI has Windows 953/six skips, twelve native-input cases and six native lab runs/three expiry checks independently verified; macOS packaging continues. No release.

- `2026-09-25`: #174 isolated preview exploration on clean 0658716 completes 28 public calls (22 MCP/six CLI), 17 positive checks and nine viewed images with independent pixel counts, source hashes and owned cleanup. It confirms #201 wrong animation time sampling and #202 ignored baseline build options; their original artifacts are retained in preview-campaign-01. #202 is now the sole active issue for focused failing regressions and existing flag forwarding; #174 review, #201 ready. Prior #200 full 108adc5 CI 36141416097 continues. 38 unique defects/27 closed. No release.

- `2026-09-25`: Full 4ea4d02 CI 36136730284 succeeds across all six jobs. Downloaded TRX verifies Windows 946/six skips, macOS 945/seven skips, Linux 33; native input five passed and all six native lifecycle runs/three expiry checks verify owned cleanup at 1x. #199 diagnostic change is validated but original UIA cause remains unknown/open. Fresh combined CI 36141416097 runs on 108adc5 including #200 cleanup 7f8e46a; #200 stays review. Both workflow integration rounds remain verified (standalone 27 calls/20 checks, Direct 26/20, six images each). #174 sole active; next isolated preview/reload/variants/baseline exploration. No local native/test process remains; 36 unique defects/27 closed, no release.

- `2026-09-25`: #174 Direct workflow comparison on clean e91041b completes 26 public calls (20 MCP/six CLI), 20 checks and six viewed images; same positive/negative execution/export/replay guards and state/counter outcomes as standalone, with no new product defect. Current-source Direct host hashes and six source hashes verify, client 396 exits 0 and owned app 3220 terminates after final reset. Both scoped summaries are retained; preview/reload/baseline and broader capability coverage remain next. Downloaded 4ea4d02 Windows TRX has 946 passed/six skipped, native input five passed, all six native lifecycle runs/three expiry checks verified; Linux passes, macOS packaging continues. #174 sole active, #199/#200 review, 36 defects/27 closed; no release.

- `2026-09-25`: #174 standalone workflow/export/replay journey on clean d4b376e passes 20 checks across 27 public calls and six viewed images. CLI/MCP positive/negative routes match independent state; complete recording/parameter rebinding, no-effect validation/guards, stop-on-failure, final reset, six unchanged source hashes and owned cleanup retained. Two agent input/parser errors are classified separately. Client 3100 exits 0; owned app 11044 terminates. Fresh Direct host publish is running. Full 4ea4d02 CI passes Windows and all native labs; Unix downstream gates continue. #199/#200 review, #174 sole active, 36 defects/27 closed; no release.

- `2026-09-25`: #200 is review at pushed 7f8e46a after 65 focused checks and one actual native integration pass; #199 remains review with original provider cause unknown. #174 is again the sole active issue for fresh workflow execution/export/replay, then remaining preview/baseline families. Full 4ea4d02 CI 36136730284 continues; start the combined 7f8e46a-or-later gate after it finishes to retain its evidence. No local owned native/test process remains, 36 unique defects/27 closed, no release.

- `2026-09-25`: #200 controlled cleanup baseline fails both variants (30 s); corrected validation passes 65 checks/one gated native skip (25 s), then actual native CLI/MCP integration passes (32 s). All three audits compare 13 OS nodes; owned host 22000 exits and is disposed, both logs and safe phase evidence persist. Exit wait is 1.22 ms, stdout completion follows 2438.88 ms later within its unchanged three-second bound; the earlier delayed-EOF cause remains unknown. Seven initial after failures were a separately retained agent fixture assumption about Process.Disposed, corrected via public disposed getter semantics. Candidate changes test cleanup only. #200 active until commit/native handoff; #199 review/full 4ea4d02 CI running, #197 done. 36 defects/27 closed; no release.

- `2026-09-25`: #197 closes after full c6b11db CI 36130672354 succeeds: Windows 937/six skips, macOS 938/five, Linux 33, all six native lifecycle runs/three expiry checks with verified cleanup. #199 4ea4d02 passes 58 affected cases and fresh standalone native exploration (nine MCP/two CLI, 14 checks, two viewed images, unchanged state/source and owned cleanup). Its original integration case has successful initial/CLI/MCP audits but FAILS at cleanup stdout drain (38 s); retained original artifacts create #200. #199 review/original UIA cause unknown, full CI 36136730284 running; #200 sole active for bounded cleanup evidence and primary-error preservation. 36 unique defects/27 closed. No release.

- `2026-09-25`: #199 controlled Windows COM baseline fails at missing diagnostics (0 pass/one fail, 178 ms); it is not a reproduction of the original hosted UIA cause. The candidate retains operation stage, actual failed HRESULT independently of IErrorInfo, elapsed/cancellation/budgets and node counts, including mapped managed exceptions and balanced COM initialization. All 58 affected accessibility/evidence-policy checks now pass (6 s, one separately gated native skip, clean build); an initial wrong UIA_E_TIMEOUT mapping assumption is retained separately (56 pass/one fixture failure/one skip). Native owned CLI/MCP verification and full CI remain next; no readiness/budget change or original-cause fix is claimed. #199 sole active, #197 review; no release.

- `2026-09-25`: #197 c6b11db moves to review after all 100 affected regressions and fresh native Direct/standalone evidence pass. Each native run has 17 public calls/seven viewed images; Direct 16 checks, standalone 17, unchanged journals/six source hashes, truthful depth/missing diagnostics and original priority restoration. Clients 6148/5104 exit 0, owned apps 20308/15968 terminate; one standalone verifier-scope mistake is retained separately without repeating app requests. HTML source passes, visual viewer remains blocked. Full CI 36130672354 continues; no combined pass claimed. #199 is sole active for safe bounded native UIA stage/HRESULT/timing diagnostics before evidence-based readiness decisions. #196/#198 completed; 35 defects/26 closed, #181 unresolved. No release.

- `2026-09-25`: #197 candidate passes all 100 affected mutation/pseudo-state regressions (32 s, zero failures/skips, clean build). The first after run (99 pass/one invalid legacy fixture input) is retained separately; its valid Visual tree kind is corrected, with optional generation metadata still absent. Unique scoped generation matching now supplies logical evidence summaries, with bounded stage diagnostics for unavailable/ambiguous captures. Native Direct/standalone verification is next; no native/full-gate pass is claimed yet. Full 8803271 CI 36125594904 is successful: downloaded Windows 923/six skips, macOS 924/five skips, Linux 33 plus all six native integrations/three expiry checks. Acceptance closes #196/#198; 35 defects/26 closed, #197 sole active, #181/#199 unresolved. No release.

- `2026-09-25`: #197 prepared baseline reproduces one logical-route failure/one visual pass (5 s) on unchanged 8803271 production; an initial canonical brush-string expectation error is retained separately. The real Avalonia MCP regression proves shared public generation identity and actual red/blue pixels. The candidate fix maps only unique scoped visual identities from the mutation response and emits bounded stage-specific diagnostics for missing/ambiguous capture; 13 protocol cases cover foreign/stale/unscoped/ambiguous/depth-limited identities and legacy visual requests. Affected mutation tests are running, no after pass yet. Downloaded 8803271 Windows TRX verifies 923 pass/six skips; all six hosted native integrations plus three expiry checks verify clean identity and owned cleanup at 1x. #198/#196 review, #197 sole active; #181/#199 remain unresolved; no release.

- `2026-09-25`: #198 8803271 moves to review after real native timelines pass in Direct (33 public calls/16 checks) and standalone (33/17), each with seven viewed images, unchanged source hashes, original StyleTrigger/Local bases, ongoing animation through individual/reset_all reset, refusal before effects and fixture reset. Both clients exit 0 and owned apps 2032/6152 terminate. Full CI 36125594904 passes Windows Build/Test/Pack; remaining platform validation continues. #181/#199 remain open despite passing Windows checks. #197 is now the sole active issue: retain a current failing logical-target evidence regression and resolve public generation identity without rewriting node IDs, then CLI/MCP/native validation. 35 defects/24 closed; no release.

- `2026-09-25`: #198 local implementation checkpoint passes 87 mutation/pseudo-state regressions and, after correcting the new fixture's closing lifecycle, all 16 QA fixture/runtime cases (33 s). Cancellation now occurs at Closing before style/animation detachment; Closed remains a cleanup fallback. The final fixture-only build reuses already validated unchanged project dependencies; Direct/standalone hosts are republished under host-animation-*-02. Earlier failed runs/agent expectation errors remain retained, and the passing later #181 assertion does not resolve its reopened lock. Full CI and native public animation comparison are next. #198 sole active; 35 defects/24 closed; no release.

- `2026-09-25`: #181 is reopened/ready after the expanded #198 run reproduces the same screenshot-lock boundary at its exclusive-open assertion (line 86, 01-normal.png); handle owner remains unknown. Counts are 35 unique defects/24 closed. Fixture lifecycle validation retains two separate agent expectation corrections: class selectors use StyleTrigger, and closed windows detach Style brushes. Initial runs are 99 pass/four fixture-precondition failures, then 101 pass/one fixture-close expectation and the independent #181 recurrence. A fresh corrected run is pending. Direct/standalone hosts and the fresh provider build; provider manifest SHA-256 54a265f39e20b338c7d7bd9c2ff31868497ac34c8d79538e2e3552d16d6baa82 verifies all 35 files. #198 sole active, native comparison next; no release.

- `2026-09-25`: #198 correction passes all 87 affected mutation/pseudo-state cases (zero failures/skips, 1m15s); the expanded before baseline remains 18 fail/six pass. The QA Rendering tab now has finite real brush timelines, independent Style/Local target/reference pairs, sampled priority/value journal and stop/reset/cleanup/close cancellation. New fixture lifecycle tests and native Direct/standalone checks are in progress; no native or full-gate pass is claimed for this change yet. #198 remains sole active, #196 review, #197/#199 ready; no release.

- `2026-09-25`: #198 expanded before-fix matrix has 18 failures/six passes across 24 cases, including animation starting after mutation and an independently animated untouched reference. Initial public-API correction is under local validation: refuse already animated writes; restore known non-local bases via same-value SetCurrentValue/ClearValue without replacing the animation; do not reapply a current-value override invalidated by that active animation. No passing after result claimed yet. Full 5fda3f9 CI 36118971656 is terminal failed: Windows main suite 895/six skips and all three native-lab jobs pass, but the separate Windows UIA audit returns unavailable with no native nodes (#199 ready); Linux/macOS downstream gates skip. Exact initial.json/logs and TRX retained under ci-5fda-windows. #198 sole active, #196 review, #174/#197/#199 ready; 35 defect tickets/25 closed. No local native app remains. No release.

- `2026-09-25`: #198 is the sole active issue: nine real-Avalonia animation-priority cases all fail on unchanged 5fda3f9 production (local/style/local-binding origins, individual/reset_all/deactivation). Reset reports success but temporary green remains hidden beneath animation and becomes visible at completion. Original TRX/log retained in animation-priority-before-results; official 12.1.3 ClearValue source confirms it skips animated effective priority. #196 moves to review with ordinary precedence/native checks passing, while full CI 36118971656 continues; animated correctness remains explicitly unresolved under #198. Expand the baseline to animations beginning after mutation and current-value sources before selecting a public-API fix/refusal. #174/#197 ready; 34 defect tickets including this discovery, 25 closed. No release.

- `2026-09-25`: Full c1f11d6 CI 36113144014 completes successfully: downloaded Windows 865/six skips, macOS 866/five skips, Linux 33, all six native integrations and three expiry checks pass; owned cleanup and clean identities verified at 1x. Acceptance audit closes #179/#181/#183/#185/#186/#188/#189/#190/#193/#194/#195, retaining original-cause uncertainty where the correction is test/diagnostic only. #182 stays review with unproven original startup cause. #196 5fda3f9 passes fresh native Direct and standalone (each 26 public calls, nine checks, four reviewed images), clients exit 0 and apps 9180/2064 terminate. Classified agent setup/evidence-routing/Stop-redirection errors are retained separately. New full CI 36118971656 runs on 5fda3f9. #196 remains sole active for the remaining animated-priority audit; #174/#197 ready. Gmail status sent at 11:11 Budapest is a historical snapshot (33 defects, 14 closed); current closed count is 25. No release.

- `2026-09-25`: #196 passes all 63 affected mutation/pseudo-state regressions (36 s, clean build), including the 30-case precedence matrix. Prepared baseline was 15 fail/15 pass; the first unprepared 24-case run retains three agent fixture errors separately. Reset now distinguishes true local priority and current-value overrides, preserves original precedence and allows later style/theme/binding/inherited changes. Native Direct/standalone verification remains next. Current c1f11d6 hosted Windows 865/six skips and all six native lifecycle runs pass with verified clean identity/owned termination at 1x; macOS/Linux downstream gates continue. #174/#197 ready; no release.

- `2026-09-25`: #196 is the sole active issue after the clean bfd7978 native mutation-evidence journey confirms two defects: reset promotes styled values to LocalValue and freezes subsequent theme updates (#196); logical mutation evidence loses target summaries despite a captured visual alias (#197, ready). The 18 public requests (16 MCP/two CLI), two passing and three failing checks, independent journal, reviewed native/rendered pixels and immutable source hashes are retained in capabilities-direct-evidence-01. Client 12144 exits 0 and owned app 21428 is terminated. First reproduce/fix #196 across value precedence and cleanup paths, then native comparisons. #174 ready; full c1f11d6 CI 36113144014 continues. No release.

- `2026-09-25`: Full CI 36113144014 is running on c1f11d6. #195 moves to review after 138+1 local cases pass; #194 also remains review with both native comparisons retained. #174 is again the sole active issue: continue native mutation evidence/visual-report and remaining capability families while the combined gate runs. Prior 511312a CI remains failed as #195, and unavailable Retina/other coverage is not converted to a pass. No local native/test process remains; no release.

- `2026-09-25`: #195's 15 initial frame/dimension preparations pass all 138 affected regressions (4m18s, clean build); the separately named MCP mutation-evidence case also passes (4s). All five measured fixtures initially lack a frame; preparation takes 2.72-4.16ms and the unchanged readiness probe 7.95-34.43ms. The original macOS cause remains unproven; the before baseline passes five cases with the preparation gap recorded. Production deadlines and mid-test/intentional readiness behavior remain unchanged. Fresh hosted confirmation is next. #194 remains review after both native comparisons and affected regressions pass, with one Direct agent mistake retained. No local native app/client/test process remains; no release.

- `2026-09-25`: #194 5d0df14 moves to review after native Direct (36 public calls/13 passing checks/one retained agent mistake) and standalone (28 calls/11 passing checks) confirm order refusal, alias identity, independent class/resource names, reverse reset and reset_all restoration. Actual values/counters, public history, viewed pixels and unchanged source hashes agree; both clients exit 0 and owned apps 12228/4476 are terminated. Full prior 511312a CI 36107667148 is failed solely on a different macOS precondition fixture readiness assertion before its test body: new #195 is the sole active issue. Windows 856/six skips, Linux 33 and all six native integrations pass; macOS 856/one failure/five skips, with #193's five and #185's affected cases passing. Preserve and measure #195, inspect similar fixture preparation, then fresh combined gate and remaining #174. No release.

- `2026-09-25`: #194 remains the sole active issue. All six before-fix real-Avalonia cases reproduce unsafe property/class/resource resets. The minimal guard uses actual owner/value identity across visual/logical aliases and returns an actionable reset-order conflict with zero effects; independent values remain resettable. All 27 affected mutation/evidence/review tests pass (20s, clean build), including nine ordering cases covering individual reverse reset, reset_all and deactivation. Native Direct/standalone verification and a fresh combined gate remain pending. The original native failure and six failing regressions are retained. Prior 511312a CI 36107667148 passes Windows and macOS/Linux native jobs; remaining jobs continue. #174 ready, #193/#185 review, #184 done. No release.

- `2026-09-25`: Full CI 36107667148 is running on clean 511312a. #193 moves to review after 39 affected local cases pass; original hosted cause remains unproven. #174 resumes as the sole in-progress issue with fresh native mutation/evidence journeys, then remaining capability families. Prior Direct table round has 40 calls/26 checks and owned cleanup; standalone table/form has 68 calls/40 checks plus three retained agent mistakes. #184 is completed; #185 review and #191/#192 ready. No active local QA/test process and no release.

- `2026-09-25`: #193's test-only frame preparation passes all 39 affected action/readiness/picking/explicit-input/gesture cases (1m13s, clean build). Initial instrumented five-case baseline passes (18s) but three valid layouts lack a frame; original hosted readiness cause stays unknown. The public headless barrier now requires real 320x240 pixels before the unchanged 1000ms probe and adds bounded phase evidence plus rendered/layout assertions. No production/deadline/retry changes. Hosted full validation is next; #174 remains ready, local QA/test processes finished. #184 is completed at 8bf3c4b after its full Linux gate and both original/queued Unix cases pass on Linux/macOS. #185 remains review, #191/#192 open. No release.

- `2026-09-25`: #174 Direct table comparison on clean e3cf363 completes 40 public calls (29 MCP/11 CLI), 26 passing checks and four reviewed images; valid/invalid edits, explicit recovery, privacy, reset/stale and historical replay agree with independent state. No new product defect in this scoped round; client 15468 exits 0 and owned app 360 is reset/terminated. CI 36102961992 on 483fedb fails only macOS fixture readiness (new #193): Windows 856 plus six skips, Linux 33, macOS 856/one failure/five skips. #184/#185 affected Unix checks and all six native labs pass; macOS downstream workflow/package gates are skipped. #191/#192 remain open despite passing hosted cases. #193 is now the sole in-progress issue: retain pre/post readiness evidence and evaluate explicit public headless rendering before changing the fixture; #174 is ready for its remaining families. No active local QA/test process and no release.

- `2026-09-25`: #174 standalone table/form campaign on clean 483fedb completes 68 public requests (50 MCP/18 CLI), 40 passing checks and three classified agent mistakes. Typed 200-row reads/filter/page, real Unicode/numeric editor changes, sort/privacy, invalid draft, explicit row finish, reset/stale/historical replay and form validation/partial repair agree with journals and viewed pixels. Escape clears a cell but may leave its row transaction active; observed valid-state plus explicit Enter recovery is documented. No new product defect confirmed in these scoped rounds. Clients 19888/20964 exit 0 after 23/27 MCP calls; final reset and owned Stop terminate app 8208. Source-backed coverage remains scoped; Direct table and remaining families are next. Instrumented CI 36102961992 continues on 483fedb (Windows Test step has passed); #191/#192 remain ready, #184/#185 review. No local QA/test process and no release.

- `2026-09-25`: Instrumented full CI 36102961992 is running on clean 483fedb. #191 remains open/ready at a diagnostic checkpoint, #192 is ready for lifecycle-phase evidence; neither is claimed fixed. Resume #174 as the sole in-progress issue with a fresh native standalone table journey. Its immutable run selects 483fedb, Avalonia 12.1.3 and the unchanged validated host/provider; startup and native outcomes remain pending. The prior Direct form round is retained at b6022e8. Shared-host contention is measured and unrelated processes remain untouched. No release.

- `2026-09-25`: #191 diagnostic checkpoint: unchanged Unicode cases pass locally (2/2, 53s), initial instrumented persistent-client cases pass (6/6, 1m54s; input spans 0.85-131ms). New controlled post-edit cooperative-budget regression passes (2.658s), proving unchanged uncertainty/exact-replay/no-repeat guards. Expanded group is **15 pass/five fail** (6m19s): MCP connect/health/response/exit waits under observed 100% shared-host CPU load, tracked separately as #192. Independent state distinguishes zero dispatch from a completed file edit awaiting exit. No production/deadline changes; original hosted #191 cause remains unknown. Full 3b82d1d CI is terminal failed on its two Windows cases; all three native labs pass with verified clean identity/owned cleanup at 1x, Unix downstream checks skipped. #191 remains active for instrumented hosted validation; #174 ready, #184/#185 review. No active local QA/test process; no release.

- `2026-09-25`: #174 Direct form campaign on clean 3b82d1d retains 32 public calls (23 MCP/nine CLI), 29 passing checks and three explicitly classified agent mistakes. Unicode fill, no-submit semantics, replay/conflict, separate popup selection, saved-state preservation, validation/partial repair and reset agree with journals and reviewed native/rendered pixels. Cross-window form selection remains a documented partial-coverage boundary. Client 21196 exits 0; owned Stop terminates app 21188. Concurrent full Windows CI 36100032601 fails two post-dispatch desired-state budget cases (853 pass/two fail/six skips); logs/TRX are retained as #191. Linux/macOS are skipped, so #184/#185 after-fix gates remain pending; native lab jobs continue. Return #174 to ready and select #191 for measured diagnosis. No active local QA/test process and no release.

- `2026-09-25`: #185's test readiness correction passes all 36 affected gesture/explicit-input/picking/action-explanation cases (one minute, clean build). Initial local diagnostic run already has a frame and passes (9 s), so is not the hosted reproduction. Official Avalonia 12.1.3 source ties hit data to full rendering; the fixture now explicitly captures/requires a 500x430 frame and slider center hit before input, records bounded pre/post diagnostics and checks one range change. Production guards/deadlines are unchanged; Linux/macOS CI now retain TRX. #184 remains review at 8bf3c4b (35 local pass/one Unix skip; controlled before-fix failures on both Unix hosts). Move #185 to review pending the combined hosted gate; begin full #174 capability campaign. Prior 8bebfe6 CI is terminal failed only on Unix queue/#185; Windows/all native labs passed. No local QA/test process and no release.

- `2026-09-25`: #184 fixture fix follows confirmed hosted before-fix reproduction on both Unix hosts in 8bebfe6 CI 36096369354: Linux queued_response reset at 206 ms (32 pass/one fail), macOS empty reply at 287 ms (854 pass/two fail/five skips); fixtures remain alive and no acquire is retried. macOS also repeats #185's first slider refusal (12 ms). Windows Build/Test/Pack and all three native QA jobs pass; overall CI is failed. The fixture now preserves a next pending pipe before disposing each accepted reply, matching production; no bridge/deadline/retry changes. Explicit fixture build is clean (21 s), rebuilt-binary local validation passes 35/one Unix skip (2m09s). Initial old-fixture run is retained separately and not counted after-fix. Move #184 to review pending hosted Unix/combined verification; select #185, then full #174. Native Direct/standalone ownership comparisons remain 24/34 passed checks, owned apps/clients gone. No active local QA/test process and no release.

- `2026-09-25`: #190 b70e35c passes all 25 affected launch/scenario/ownership/recovery cases (2m16s, clean build), including three actual CLI/PowerShell failure/repeated-cleanup/PID-identity guards. Original native standalone-01 recovery exits 0, cleaned/already_exited, retaining failed/top_levels and unchanged startup/Stop hashes. Fresh clean cleanup-standalone-01 passes ten retained native checks: Win32/Avalonia 12.1.3/1x, readiness, unchanged capture state, reviewed Windows/rendered images, one toggle/reset and owned PID 9956 termination; client 1828 exits 0 after three calls. Bootstrap latency remains unexplained. #190 moves to review; #184 is next, then #185 and full #174. #189 remains review at cef45b7; no active local QA/test process, no release.

- `2026-09-25`: #189's test-only lifecycle correction passes all 13 affected launch/scenario cases (2m53s), then the final CLI case again (11s, clean build) after diagnostic/policy cleanup. The unchanged controlled launcher probe records CLI exit at 3995ms, streams closed at 5306ms and the owned child alive until explicit release at 32755ms; no product wait defect or original 7.767s cause is claimed. The new guard independently checks CLI exit, output completion and PID/start-time-verified child liveness/cleanup, keeping the actual 500ms attach timeout. #189 moves to review pending the combined gate; #190 is next, then #184/#185 and full #174. #188 remains review at 92c5f10/7739a87: 59 regressions and both native 22-check rounds pass; its first standalone startup failure remains #190. No native QA/helper process remains; no release.

- `2026-09-25`: #186 implementation and scoped native verification are complete, pending a passing combined gate. Final focused pointer validation passes 19 cases (32 s); full Release build has zero warnings/errors. Full local Release finishes with 838 passed, one failed and five native skips (35m05s), solely #189's launch elapsed guard (7.767 s versus <4 s); no launcher cause is inferred or threshold changed. Fresh isolated pointer-standalone-01 passes 18 retained checks and pointer-direct-02 passes eight popup checks: actual hit paths, marker pixels, mapped local coordinates, ambiguous-root refusal, negative assertions and independent journal/counter agreement. Original Direct failure evidence and separate #188 mismatch remain retained. All owned apps/clients are reset/stopped (standalone app 11868/client 4444, Direct app 11992/client 21184). Move #186 to review, select #188 next, then #189/#184/#185 and #174. Native Retina and original timeout gaps remain. No release.

- `2026-09-25`: #186 is the sole in-progress issue after #187 closed at `91819d1`. The real-Avalonia regression fails before the fix; all 17 affected picking/Core/CLI/protocol cases now pass (36 s), including transformed/clipped/overlaid controls, partial/unavailable evidence, explicit cross-root ambiguity and mapped overlay pixels. Full Release build is clean. Native `pointer-direct-01` confirms the corrected pad path, clear-area CLI/MCP positives, negative assertions, native click/drag counters and stale refusal. Center movement exposes separate legacy hit-test-invisible dispatch #188; the genuine mismatch stays reported. A popup disappeared between geometry and input, so that attempt is incomplete; bad activation-revision/unsupported menu requests were safely refused and are recorded as agent-authored corrections. Direct reset/owned PID 12868 termination passed, client 2964 exited 0 after 23 calls. Full Release suite is still running (pointer-full-release-results); stack/process evidence shows active preview work, not a proven hang. Standalone and isolated native popup validation remain. #188, #184/#185 and full #174 campaign are queued. No release.

- `2026-09-25`: #187 validation is complete: strict UTF-8 pipe decoding preserves raw accents/CJK/emoji without changing console code pages; malformed/invalid-byte input has bounded failures. All 8 focused client/AgentRecipe Release cases pass (2m28s); initial test cleanup/self-join and single-line fixture mistakes are corrected with failed evidence retained. Fresh native `utf8-direct-01` passes raw multiple requests, exact replay with unchanged journal and file mode, independently checked through state, Windows accessibility and viewed pixels. Node cp852 is unchanged before/after; no escape workaround. Five retained checks pass; app PID 9664 terminated and client 1032 exited 0 after six calls. Original old-client failures remain separate. Complete #187 and select #186 next, then #184/#185 and #174. Broad combined gates and real Retina remain outstanding. No release.

- `2026-09-25`: #173 fixture slice at 5e5f159 is validated by 25 passing regressions and fresh native Direct/standalone Runtime journeys. Scene query/action/shift/stale/reset, native selection, typed work progress/complete/fail/cancel, declared cleanup/prepare with no late journal writes, runtime assertions, validation/clipping and reversible mutation images pass. Direct traces correlate operation completion separately from other failures; bounded validation sampling is partial and binding/log/event adapters remain unavailable, explicitly not passed. Inline mutation trees are truncated with full artifacts retained. Independent retained-evidence checks: 13 standalone, 19 Direct. Both apps reset and owned PIDs 20092/15012 terminated; persistent clients 21488/20920 exited 0 (39/37 calls) using the explicit #187 Unicode-escape workaround. No new product defect confirmed in this scoped round. Complete #173 and select #187 next to remove that test-client workaround, then #186/#184/#185 and #174. No local test/QA process remains; broad gates and native Retina are still outstanding. No release.

- `2026-09-25`: #173 is the sole active issue. The shared Runtime tab adds three custom-drawn records with app-owned identity/revision/selection, bounded cancellable work, and opt-in validation/binding/layout diagnostics. Direct registers declarations on the scene's first visual attachment; pure standalone keeps ordinary fixture UI and reports declarations unavailable. The initial new regression caught premature registration of detached tab content (2 passed/1 failed); that fixture wiring is corrected. All 25 fixture/scene/operation Release tests now pass (67 s, runtime-fixture-results/runtime-fixture.trx), including exact production registration, transformed pointer selection at two sizes, stale/reset tokens, typed parameter refusal, completion/failure/cancellation, and no journal writes from old work after reset/cleanup. Both host publishes pass without warnings. Native public-tool validation remains in progress; no passing native claim yet. #184–#187 remain queued and the full campaign is unfinished; no release.

- `2026-09-25`: #172 fixture expansion at 8e36e2a has passed 12 fixture/virtual-item Release tests (30 s) and both pure standalone/Direct publishes without warnings. Fresh native Windows `keyed-standalone-01` and `keyed-direct-01` V/K/D journeys verify keyed offscreen list, reset, focus/action map, native Unicode/Tab/Shift-Tab/Enter, click versus 80x30 DIP drag, replay and no-event refusal against journals, Windows accessibility and reviewed pixels. Two distinct failures are retained: both pointer-diagnostics routes choose a background border (#186); Direct persistent test-client raw UTF-8 input corrupts text under cp852 (#187), with R004-R006 using an explicit JSON-escape workaround. Public pick independently identifies the correct pad. Both apps reset and owned PIDs 20960/21724 stopped; persistent client 21152 exited 0 after 31 calls. Details in AGENT_QA_PHASE_ONE_REPORT.md. #172 fixture implementation and its scoped validation are complete; #173 is next, while #174 owns comprehensive validation and #184–#187 require fixes. #179/#181/#182/#183 remain in review. No active local test/QA process, no fresh CI yet, no release.

- `2026-09-25`: #183 52f17c0 passes actual Windows paired CLI/MCP capture (1/1, 38 s) with clean/occluded/popup/masked/offscreen evidence and owned cleanup. Six native/rendered images were reviewed; actual native output is Win32 BitBlt 400x300, while full 3840x2160 coverage is the controlled 29-test screen/privacy run. Both are explicit, not a native Retina claim. #183 moves to review for combined platform validation. CI 36072375163 on b90ca39 is terminal failure: Windows/all native labs passed, Linux recovery conflict failed (#184, 28 passed/1 failed), macOS slider hit-test refused (#185, 816 passed/1 failed/5 skips). Logs retained under expanded-campaign/ci-b90-*. #179/#181/#182 remain in review; original one-off causes are not inferred from unrelated passing cases. No active test/QA process. Resume unfinished #172, then #173/#174 and remaining queued bugs. No release.

- `2026-09-25`: #183 is the sole in-progress issue. Unchanged processing reproduced comparison timeout at 5151 ms/8202240 of 8294400 pixels, losing both frames (screen-before: 3 passed/1 failed). Reordering only preserved both frames at the same deadline while comparison still timed out (screen-preserve: 3 passed/1 comparison failure); this intermediate failure is retained. Public bulk Skia colors plus cached dimensions now pass all 29 screen/privacy guards (screen-after-results/screen-after.trx, 31 s), including last-pixel, alpha, tolerance-boundary and invalid-frame cases at full 3840x2160. Valid-case total test duration is 1.88 s versus about 6 s before; no deadline increase. Additional color arrays remain bounded at 64 MiB by the existing pixel limit. Save diagnostics retain safe stage/timing/cancellation details. Native paired CLI/MCP validation follows. CI 36072375163 on b90ca39 has passed Windows/all native labs but failed Linux recovery (#184) and macOS (log under inspection); do not call the workflow passed. #179/#181/#182 remain in review; continue unfinished #172/#173/#174. No release.

- `2026-09-25`: #182 bdd2e9d passes fresh native Direct and standalone startup: bridge/application/frame conditions all matched before initial tree, app journal remained seeded, and public capture-after-render plus reviewed native pixels agree at Win32/Avalonia 12.1.3/1120x800/1x. Both owned processes (15148/7380) are terminated. Evidence: startup-ready-direct-01 and startup-ready-standalone-01, D001-frame and native-ready.png/ready-frame.png. Reused unchanged 47034aa bridge/provider and fixture; client snapshot is bdd2e9d. Layout sampling remains explicitly truncated, not full stability evidence. #182 stays in review for broader validation/original cause uncertainty. #181/#179 remain in review; CI 36072375163 on b90ca39 has passed its Windows Test step and continues packaging/native jobs. Next sole implementation issue #183, then unfinished #172/#173/#174. No local test/QA process is running; no release.

- `2026-09-25`: #182 is the sole implementation issue. The lab now requests existing bridge/application/frame startup readiness before initial inspection; lifecycle validation checks all three retained observations. Transport failures retain bounded phase/attempt/elapsed/timeout/received-byte diagnostics and tree depth; no operation timeout/retry policy changes. Focused Release validation has 58 passed/1 failed (startup-diagnostics-results/startup-diagnostics.trx, 4m05s): both controlled timeout/recovery cases, actual readiness-before-tree and lifecycle guards pass; the unchanged valid 3840x2160 screen-pair case returned rendered=unavailable and is retained separately as #183. Do not call the whole run passed. Original native startup timeout cause remains unproven; fresh native starts follow. #181 diagnostic slice b90ca39 and #179 remain in review; full CI 36072375163 runs on b90ca39 and must not be cancelled for subsequent work. Continue #183 and unfinished #172/#173/#174. No release.

- `2026-09-25`: #181 ownership/diagnostic slice passes all six matrix/diff tests in matrix-ownership-results/matrix-ownership.trx (7 s, no warnings/errors). The existing IPC matrix regression now requires immediate exclusive read/write access, decodable pixels and rename/restore for all four screenshot/diff/contact-sheet artifacts before cleanup, without GC or retries. Failed cleanup retains every attempt's elapsed time, HRESULT and message; the existing five attempts/delays are unchanged. This is not an established fix for the original hosted lock. Production code remains unchanged; #181 stays open pending a fresh full hosted gate and cause evidence. #178/#180 are closed; #179 stays in review; #182 and unfinished #172/#173/#174 follow.

- `2026-09-25`: #180 is validated at 47034aa: fresh native Direct and standalone P001-P016 pass menu/context provenance, paired cleanup, wrong-owner/disabled/stale refusal, independent once-only counters, reset and owned termination. Both retained Win32/PopupImpl/HWND/1x evidence after closing; reviewed native images agree. Standalone used verified provider manifest 3d6059c6feb571d83a7bd69954c9e9dac72e8d5d2c5b1f019a74322ac66a6b9f. Full Release build and 35 existing plus one closing-window guard passed. #178 is closed after CI 36068144424 completed all three native labs (both integrations, two cycles, cleanup/expiry); all observed scales are 1. That workflow is overall failed: Windows 815 passed/1 screenshot cleanup failure/5 skips, tracked separately in #181; downstream full macOS/Linux gates were skipped, so #179 stays in review. Direct's initial attached-app tree timeout is retained as #182; subsequent read-only recovery and input journeys passed without restarting, not a passing startup. No local QA app remains running. Next #181/#182, then unfinished #172/#173/#174. No release.

- `2026-09-25`: #180 is the sole implementation issue. Its pre-dispatch provenance capture passes 35 existing input/focus/precondition/picking/action/bootstrap guards plus the corrected closing-window/paired-KeyUp/validation regression (provenance-closing-results/provenance-closing.trx). The first new-test assertion wrongly expected an explicit registration to disappear before its helper disposed it; current backend unavailability is now checked separately from the retained dispatch backend. No additional product failure is claimed from that test-authoring mistake. Full Release build passes with zero warnings/errors. Read-only provenance-original-direct.json confirms both retained 2a80ba1 menu/context replies lose Win32 evidence. Fresh native Direct/standalone validation follows; expanded native CI now compares popup provenance with the observed pre-action surface. #179 e2f436d passed all 18 expression/IPC/CLI/MCP tests and remains in review with #178; full CI 36068144424 is running on that exact commit, so do not cancel it for later work. Continue unfinished #172/#173/#174 afterward; original #179 poll sequence, #171 original timeout and real Retina gaps remain explicit. No release.

- `2026-09-25`: #179 local validation passes all 18 expression/IPC/real CLI+MCP tests in expression-final-results/expression-final.trx (35 s). Controlled pre-fix probes failed 2/3; the fix retains original operand timestamps across an unavailable poll and explicitly reports no completed sample, without changing deadlines. Real UI variable substitution uses a supported single-observation branch plus a successful wait; the former subsecond timeout assumption is covered separately by three controlled IPC deadline cases. Earlier expression-after had 17 pass/1 initial headless readiness failure; added diagnostics, then corrected an invalid test-authored assert_state request to the supported expression branch. Those failed attempts remain retained and are not additional product reproductions. The final complete focused run passes without warnings/errors. Full CI follows; #178 remains in review. Previous 2a80ba1 CI completed Windows/Linux/macOS package/test gates (macOS 813 pass/5 native skips); its native labs were cancelled at 25 minutes, so repeat them with the committed 40-minute budget. Original hosted #179 poll sequence remains unknown. No local QA app is running; no release.

- `2026-09-25`: #178 native CI 36062837820 exceeded its original 25-minute job budget. Downloaded Windows/macOS/Linux evidence confirms all six integrations completed both menu/context cycles; Direct completed all 100 calls everywhere, standalone completed 100/86/95 calls respectively before cancellation. Calls average 6.4-7.2 seconds; no popup assertion failed. Windows reached expiry startup; full Build/Test/Pack passed. Increase only the outer native CI budget to 40 minutes for the expanded 200-call gate plus startup/expiry, retaining per-operation limits and all assertions. Fresh native CI is required; do not count cancelled jobs as passes. Local Direct/standalone #178 acceptance and 30 focused guards remain passed. No release.

- `2026-09-25`: #178 remains the sole active issue at 2a80ba1. Fresh clean native Direct and standalone P001-P016 both pass menu/context discovery, explicit popup input, wrong-owner/disabled/stale refusal, MCP/CLI once-only counters, reset and owned Stop. Native images were reviewed; both hosts use Avalonia 12.1.3 / Win32 1x / 1120x800. Provider manifest b1415252b808381effb9d99cfad67901f2868c85825d34bd5142f420eead325c verified. Full Release build: zero warnings/errors; focused input/focus/picking/action-explanation/bootstrap guards: 30/30. Full CI 36062837820 is still running with the expanded two-cycle native menu/context gates. #179 retains the prior macOS expression-timeout assertion failure (one current local focused run passes, cause still unknown); #180 records backend evidence lost after successful popup closure. #176 completed; #172/#173/#174 remain unfinished. No local QA app is running. Preserve Retina gaps; no release.

- `2026-09-24`: #178 is the sole implementation issue. Fresh native pre-fix P001-P004 confirms a visible menu missing from list_top_levels; both headless menu/context cases reproduce erroneous failure after Enter closes the item. Public visibility/parent discovery and owned synthetic KeyUp handling pass all 30 input/focus/picking/action-explanation/bootstrap guards, including single Enter and blocked additional Enter after menu/context closure. Native Direct/standalone verification remains pending. Native CI now includes two menu/context cycles, disabled/wrong-owner/stale refusal and independent counters. #176 is closed at 2db6dc6: both fresh native integrations plus CLI edit/replay/read-only/sort/reset and owned cleanup pass. CI 36057932181 passes Windows, all native labs and Linux; macOS has 808 pass/1 workflow-expression failure/5 skips, retained separately as #179. Continue #178, #179 and unfinished #172/#173/#174; Retina remains external. No release.

- `2026-09-24`: #176 is the sole active issue. Explicit depth-64 discovery plus preserved table coverage/pending-edit/intent diagnostics now passes the real shared-editor regression and all 34 table/query/desired-state guards. Old depth-32 targets remain safely rejected; explicit draft cancellation, fresh complete target, edit and one-write replay pass. Original full-height focused attempt hit the unchanged 3-second deadline; the depth regression now realizes fewer rows, so full-size native validation is still required (Direct04 startup is running). #175 and #177 are completed: 33 fixture/form/table checks and every job of native CI 36054889116 pass; downloaded Windows Direct/standalone images show Reset and all table columns within the actual 1028x749 client after two cycles. Mac/Linux native paths also pass; all observed scales remain 1. Explicit-provider headless lab03 passed both integrations and stopped owned processes. #178 queues visible native menu popup targeting/keyboard failure from S014-S017; independent Return worked once and standalone01 was stopped. Continue #176 native/CLI/standalone, then #178 and unfinished #172/#173/#174. Retina remains external; no release.

- `2026-09-24`: #177 is the sole implementation issue; #175/#172 remain in review. CI `36051089119` on `f394fb7` passes macOS/Linux native lab but Windows fails its second Reset: retained 1028x749 pixels show the button extending to x=1100. A focused content/client mismatch reproduces the exact x=988 origin; ordinary accepted resizing does not. The shared fixture now binds its viewport to observed ClientSize, preserving requested scene intent; 33 fixture/form/table tests pass. Fresh native standalone `expanded-standalone-01` S001-S014 renders rows, edits offscreen QA-175 once including replay, sorts, resets and exposes the same invalid-email error in UI/journal/inspect_form with password redaction. Direct03 T021-T029 explicitly finished the old draft, verified edit/replay/read-only/numeric/sort/reset, then owned Stop; this workaround does not fix #176. The first headless standalone attempt used an old default provider, so its #158 diagnostic failure is setup mismatch, not a new regression. A fresh verified provider and explicit ProviderDirectory now run `expanded-table-headless-03`; do not duplicate it. Native #177 CI remains next, then #176 and remaining #172/#173/#174. No release.

- `2026-09-24`: #172 is the sole in-progress issue; implementation slice `6cd33b2` is pushed. Shared Form/Table/Input tabs and a source-checked map of all 75 MCP tools are present. Direct/standalone builds: zero warnings/errors; fixture/form/table suite: 31/31; final layout/reset fixture checks: 6/6. Actual Direct pixels exposed and verified correction of overlapping form columns. Native `expanded-direct-02` is retained for ongoing exploration: labeled inventory/redaction, four-field fill and exact replay pass; closed ComboBox selection remains explicitly partial and other fields still fill. Table/input/standalone journeys remain pending, so this is not completed validation. #173 follows for declared scenes/operations/diagnostics; #174 owns comprehensive execution; #166 is in review. Preserve native Retina, HTML-viewer and original #171 timeout limitations. No release/version/publication.

- `2026-09-24` **Blocked handoff after completed gates.** Every job of CI `36031948678` passes on `a6783cc`. Full Windows/macOS each pass 803 tests plus five explicit native skips, with zero build warnings/errors. Windows TRX verifies the original Unicode case at 0.636 s and controlled deadline/recovery at 5.054 s. Downloaded six native lab journals verify clean identities, independent state and owned termination; Linux/macOS input/dialog checks pass 20 each and X11 paired CLI/MCP compares every 3840×2160 pixel. All jobs and local tests are terminal. #166/#171 are now open/blocked alongside #157/#161: actual native Retina access is still missing, and #171 needs a reproducible failing environment or a recurrence with the new retained diagnostics to establish its original cause. The host request remains unanswered; do not substitute headless 2× or more unchanged passing suites. Preserve original failure evidence. Resume #157/#161 using `AGENT_QA_LAB.md` when a real 2× Mac is accessible, or #171 when new failure evidence exists. Only docs follow tested `a6783cc`; commits are pushed. Product stays 1.5.0, with no release/publication.

- `2026-09-24`: #171 remains active. `a6783cc` is committed/pushed with the 12/12 passing deadline/recovery test slice. Unchanged-source CI `36025169889` attempt 2 has now passed every job: full Windows/macOS 802 plus five skips each, all six native lab combinations, Linux/macOS package gates. Downloaded Linux/macOS journals confirm 20 native input/dialog checks each; X11 paired CLI/MCP compares all 3840×2160 pixels; hosted macOS remains 1×. New full CI `36031948678` is live on `a6783cc` for diagnostic/TRX retention and the new regression (expected full-suite count 803 plus five native skips). Inspect that exact run and retained TRX next; do not duplicate/cancel it merely because an observation yields. Original timeout cause remains unknown; Retina/HTML-viewer gaps remain explicit. No local test or QA app is left running.

- `2026-09-24`: Resumed after the owner's restart pause. #171 remains the sole active issue, #166 in review. The new deterministic IPC-timeout regression passes with all 12 text-edit tests: an actual application callback stays blocked until the unchanged five-second client deadline, then the exact original request recovers its retained result with one edit/dispatch. This establishes recovery behavior, not the original hosted failure's cause. Log/TRX: `artifacts/agent-qa/timeout-boundary.log`, `timeout-boundary-results/timeout-boundary.trx`. CI `36025169889` attempt 2 on unchanged `2f3894f` has passed Windows, Linux and all three native lab jobs; macOS packaging is still running. Next: complete that run, then validate the instrumented/test-only source in full CI with retained TRX. #157/#161 still require real native Retina acceptance. No release or version change.

- `2026-09-24`: #171 remains the sole active investigation; #166 is in review. CI `36025169889` attempt 1 fails one full-suite Unicode range edit at the five-second IPC boundary (801 passed, one failed, five skips); all six native lab combinations pass and downloaded journals agree. Fifteen unchanged focused repetitions and a second complete local Release run pass (802 plus five skips, failing case now 0.208 s); hosted attempt 2 passes Test and continues through packaging. Cause remains unknown, so production timing/dispatch/retry behavior is unchanged. Diagnostic-only work adds elapsed/timeout/observed-control state and Windows CI TRX retention; a temporary one-tick negative injection verified those fields and was restored. All 17 restored-path text/observation/MCP regressions then pass without build warnings/errors. Original failure remains retained, not relabeled fixed.

- `2026-09-24`: #166 is active pending final combined CI `36025169889` on code `2f3894f`. Fresh direct native F000–F036 passes with no new defect: selector/desired-state/text replay, native/rendered templates and opacity, observe bounds/retry, all overflow-mask pixels, exact IDs, stale editor recovery, popup identity/close guard, child/modal lifecycle, loading/reset and owned termination. Nineteen independent evidence checks pass. Isolated preview P001–P015 also passes; HTML viewer browser review is blocked by the tool's file-URL policy. Full Release at `678ac0b` passes 802 plus five native skips; focused Debug 17/17. Every rendering-baseline CI `36019901130` job passes; downloaded X11 evidence covers all 3840×2160 pixels, hosted macOS is 1×. #157/#161 remain blocked only on real native Retina acceptance. No publication.

- `2026-09-24`: #170 is the sole active issue. Standard Range metadata now publishes all five observe numeric bounds; the pre-binding guard returns trusted field/minimum/maximum diagnostics for constructor and JSON numeric/type failures without caller values. The actual stdio regression failed on the absent schema minimum before the fix; 17 observation/text-edit/MCP tests pass afterward, including invalid values through absent manifests, valid endpoints/defaults and corrected retry. Native standalone confirmation follows. #157 is in review: full Release 801 passed/5 explicit native skips; Debug and full CI `36019901130` continue, with all six native lab combinations passed. Native Retina acceptance remains outstanding.

- `2026-09-24`: #157 `c8b7dc3` is the sole active issue. The opacity/transform DPI defect reproduces on actual Avalonia 12.1.0 and 12.1.3; six 1.5×/2× IPC regressions fail before the shared public VisualBrush mitigation, and 82 focused tests pass afterward. Clean native standalone O002–O017 confirms five equal rows, opacity/clipping, exact screenshot/observe/capture_screen/CLI bytes, dark/hu-HU navigation, reset and owned cleanup at actual Win32 1×. Full local Release→Debug and CI `36019901130` are running; native macOS 2× acceptance remains outstanding. New #170 queues the observed missing observe bounds/recovery diagnostic. #169 is completed after every job of CI `36014239916` passed. Product remains 1.5.0; no publication.

- `2026-09-24`: #169 `5bbaad1` is in review after native standalone M100–M134 (exact mask pixels through MCP/workflow/CLI, unmasked recovery, desired state/replay, Unicode, appearance, exact IDs, stale recovery, popup, load/reset and owned cleanup) and full Debug/Release, each 792 passed with five explicit native skips and zero build warnings/errors. No implementation issue is active. Updated full CI `36014239916` is running; its full test step passed. Baseline full CI `36009001380` passed all jobs, native 5/5 per platform and X11 3840×2160 paired CLI/MCP; downloaded evidence inspected. #158/#159/#162/#163 are completed. #157 and #161's outstanding native 2× acceptance are blocked on Retina access. No version or publication change.

- `2026-09-24`: #169 is the sole active issue. Native public MCP capture reproduced an oversized privacy rectangle overflowing its endpoints: `masking: applied` with readable pixels. Eight new memory/file cases failed before widening endpoint arithmetic; evidence/capture/client tests now pass 102/102 and the final evidence/capture group passes 64/64 including masking before IPC. Release build has zero warnings/errors; fresh public native rerun remains. Baseline `7d86ab3` Debug and Release both passed 771 tests plus five explicit native skips; the missed boundary means this is not a clean final campaign. All six final lab combinations and Windows packaging passed; hosted macOS/Linux final gates remain running.

- `2026-09-24`: Campaign validation is active; #162 is in review pending combined gates. Clean `7d86ab3` passed native standalone S301–S325: declared readiness/load/reset, popup contrast in both themes, desired-state/Unicode replay/stale recovery, case-distinct actions and independently viewed complex text at 1×. Owned cleanup passed; no new native defect found. Full Release passed 771 tests with five native-only skips; Debug build passed without warnings/errors, tests and full CI `36009001380` are running. #161 is in review at `295939d`; native 4K/Retina remain outstanding. #157 is blocked on an accessible native Retina reproduction; no speculative rendering fix was made. #168 completed after corrected CI `36006539457` passed all six native combinations and expiry.

- `2026-09-24`: #161 is the sole active implementation issue: shared 8 Mi-pixel/16384-dimension limits preserve full 3840×2160 output, with native preflight, callback bounds, unchanged encoded/IPC limits and bounded client processing. The original 2×/exact-boundary tests failed before the correction; 67 focused capture/client/capability tests and the final 20 capture tests passed. New isolated X11 4K CLI/MCP validation is pending hosted execution. #168 is in review; corrected lab `36006539457` is pending. Native Retina acceptance remains distinct from headless physical-dimension regressions.

- `2026-09-24`: #168 remains active: `d1028c3` passed 52 focused tests and native standalone agent CLI/MCP exploration (S200–S244), including unique open/closed popup identities. Hosted `36004045571` also passed open-popup queries on all three direct backends but stopped at an invalid test sequence (owner Reset while popup open); the script now checks closed then open states. Both local campaign apps were stopped through owned termination. The report records standalone Unicode/replay/stale/disabled/read-only/appearance/virtualization/modal/focus/loading/capture-boundary checks; final combined gates and true 2× Retina remain outstanding.
...
- `2026-09-24`: #158's retained native repro is a coverage diagnostic defect: depth 24 is partial and rejected, while the same complete selector at depth 32 toggles once, repeats with zero dispatch, reads text and changes the name successfully (D306–D311). The bridge now reports selection-incomplete codes with coverage reasons/bounds/recovery guidance instead of claiming a changed target. Complete selection, exact replay, already-satisfied state, policy denial, true generation/replacement and three-way ambiguity remain covered; desired-state/query/text/dispatch tests passed 34/34. Native lab regressions now contrast deliberately partial versus complete targets using independent counters. Original macOS complete-coverage behavior is not inferred from this diagnosis; native follow-up is pending.

- `2026-09-24`: #158 is the sole active issue, diagnosing complete versus depth-limited selector targets. #167 (`cfa0d3d`) is in review: native direct MCP and CLI now agree on bounds (44,236,150,32), and a geometry-pinned pick at that center hits the expected toggle (campaign-direct-03 D301–D305). Its two-host/three-platform native CI `35998419321` is running. #160 remains completed.

- `2026-09-24`: #167 now reuses legacy top-level visual bounds in structured queries while preserving logical layout bounds. Native standalone reproduced the original mismatch even with complete coverage. The nested-margin/1.15× transform/70-DIP scroll regression failed before the fix and passes with geometry-pinned picking; all query/picking tests passed 15/15. The new #160 actionable test now explicitly awaits a rendered frame, resolving an observed preparation race. Both fixed cases are added to the shared native lab check for future three-platform/two-integration runs. #167 remains active pending native verification.

- `2026-09-24`: #160 completed at `173b42b`: 14/14 focused tests and native direct/standalone public MCP + CLI reruns passed; independent counters and viewed native/rendered images agree. #167 is next, for inconsistent query bounds. Fixture `936dbed` also passed the updated two-host native lifecycle/expiry checks on all three hosted backends in `35996074830`, verified from downloaded summaries. See the campaign report for evidence and continuing platform gaps.

- `2026-09-24`: #160 exact AutomationID matching is corrected in the shared bridge matcher, pseudo-state re-resolution and audit duplicate/recommendation logic. New regressions failed before the fix (two matches for one exact ID; incorrect audit count) and the relationship/query, audit and matrix groups now pass 14/14, including real CLI/MCP lookup and selector dispatch, independent case-distinct counters, wrong-case negative, policy exclusion and genuine duplicate rejection. Native direct/standalone verification follows before closing. Fixture lifecycle validation `lifecycle-validation-03` passed both native integrations including close/reopen/reset and owned lease expiry; hosted fixture rerun remains pending.

- `2026-09-24`: #160 is the sole active implementation issue; #166 is in review while individual defects are fixed. `936dbed` corrects fixture child registration and extends native close/reopen/reset checks; focused tests passed 2/2. The baseline combined CI `35991939691` completed successfully, including Windows, Linux and macOS product/native jobs. Updated fixture lifecycle checks are running locally and in the focused hosted lab matrix. #160 starts with exact-ID matching and its related query/action/audit regressions.

- `2026-09-24`: The owner expanded #166 to stabilization: finish the lab, record and fix existing/new runtime bugs, improve meaningful regressions and repeat comprehensive testing until no new failures are found in the applicable round. This supersedes the earlier intake-only boundary; publication/version changes remain outside scope. #166 stays the sole active issue while completing the fixture lifecycle correction, then individual defects will be handled one at a time. Native Retina availability remains a validation constraint. A direct fixture incorrectly retained manually registered closed children; it now relies on desktop lifetime discovery, with new native close/reopen/reset coverage. Focused fixture tests passed 2/2; updated lifecycle validation is running.

- `2026-09-24`: #166 is the sole active issue; #162/#163 are in review. Local lab lifecycle checks passed both integrations, two cycles each, negative evidence and lease/owned cleanup. Full Release passed 745/745 with five explicitly gated native skips. CI `35991939691` passed the new native lab jobs on Windows, X11 and macOS; all observed 1×, so Retina remains blocked. Actual direct Win32 agent exploration confirmed #158/#160, narrowed a reproducible opaque MCP diagnostic for #159, and filed new #167 for inconsistent structured-query bounds. Evidence and the unfinished campaign ledger are in [AGENT_QA_PHASE_ONE_REPORT.md](AGENT_QA_PHASE_ONE_REPORT.md). Product fixes and release remain outside scope.

- `2026-09-24`: #163 is the active slice. `eng/agent-qa.ps1` now uses profile launch/ownership for retained native sessions, copies and hashes the selected tools/host/provider, preflights with target doctor, exposes individual public MCP calls with full failure envelopes and before/after app journals, and provides reset/owned stop/recovery. Direct and standalone Win32 bootstrap/reset/stop passed; direct native and RTB images were viewed at actual 1× with matching text/layout. The documented six goal-based charters are in [AGENT_QA_LAB.md](AGENT_QA_LAB.md). Scripted two-cycle/negative/lease validation and the full Release suite are running; hosted native lifecycle validation is pending. The detailed #166 defect campaign follows this infrastructure checkpoint. No product implementation/version/release changes.

- `2026-09-24`: #162 fixture implementation is ready for shared platform validation: both hosts compile the same QA XAML without a standalone AvaScope reference; native/headless selection, deterministic reset, independent bounded state/event snapshots, genuine editor replacement and a bounded session lease are implemented. `AgentQaFixtureTests` passed 2/2 after correcting a fixture-only closed-window screen query; existing complex CLI passed two clean runs plus redacted failure/retention/cleanup. Direct and standalone Win32 windows were inspected through independent Computer Use screenshots at actual 1×; standalone attach and owned termination passed. Native 2×/macOS evidence is still outstanding. #162 moves to review; #163 is the next active implementation slice. Product binaries remain 1.5.0.

- `2026-09-24`: User authorized the first implementation phase only: #162 representative test app, #163 hands-on agent workspace, then [#166 detailed exploratory QA and bug intake](https://github.com/RolandUI/AvaScope/issues/166). #162 is the sole in-progress issue; #165 returns to backlog. Product defect fixes, version changes and release execution are outside this phase. Work is on `codex/agent-qa-phase-one`; local Windows native observation is available through Computer Use, WSL is absent, and hosted Linux/macOS capability must be verified before claiming coverage. Existing intake documentation is retained at the user's request. Initial fixture build passed; validation is ongoing.

- `2026-09-24`: Planned [v1.5.1](https://github.com/RolandUI/AvaScope/milestone/20) for runtime stabilization: reported defects #157–#161, representative native sample fixtures #162, reproducible exploratory agent sessions #163 and packaged MCP/native visual gates #164; release tracker #165. See [Agent QA Plan](AGENT_QA_PLAN.md). Planning only: no fix, runtime reproduction, package-version change or release. The complex sample currently always selects Headless; existing native smoke does not establish complex text fidelity. Static inspection supports the AutomationID case-comparison and pixel-limit reports. Intake validation: issue/milestone/roadmap consistency, local documentation links and `git diff --check`; build/runtime tests are not applicable to these documentation-only changes. Next implementation issue: #162; no implementation issue is in progress after intake. Actual native macOS 2× Full HD evidence is required and not supplied by this Windows planning session.

- `2026-09-23`: Published and independently verified [v1.5.0](https://github.com/RolandUI/AvaScope/releases/tag/v1.5.0) from exact release commit `f5aaa8651e77b14f18982e662c4d23a347206bfb` through successful Release workflow `35838400133`. The remote tag matches; fresh public downloads verified all 14 assets and 13 manifest entries by filename, size and SHA-256, plus the standalone provider inventory. All three nuget.org packages expose 1.5.0 as latest, pass repository-signature verification and have unsigned contents identical to the release packages. The workflow confirms all three GitHub Packages pushes. Final Debug and exact-version Release each passed 743 tests with 0 build warnings/errors; five native-only cases passed separately on Windows/Linux/macOS. All 38 implementation issues, release tracker #156 and milestone v1.5.0 are closed. #154 remains cancelled/not planned; no foreign-framework interoperability was implemented.

- `2026-09-23`: v1.5.0 is Release Candidate. All 38 accepted implementation issues (#117–#153 and #155) are closed; #154 remains not planned. Broad CI 35832938516 passed Windows/Linux/macOS at ca0c75d (Linux passed on unchanged attempt 2 after an X11 startup timeout); downloaded evidence independently confirms native 5/5, provider/onboarding/platform checks and seven managed Wayland cases. Final build-only correction 91949f9 passed focused hosted Windows 35836813288 and macOS 35836819598; Linux 35837679713 includes test-fixture-only 4170beb. Each passed transport/build/recovery 30/30 and native 5/5. Fresh Debug and exact-version Release passed 743 tests each plus 5 separately gated native-only skips, with 0 build warnings/errors. All 13 exact 1.5.0 manifest entries, Windows/Linux installers, packaged doctor/preview/lifecycle/complex CLI/MCP, standalone positive/negative/shutdown cases, native Windows 5/5 (3433b660b73c41e6bd6e93312e72df3f), cross-published Linux CLI/MCP/provider native Wayland (release-1.5.0-25anggu8), and both publish dry-runs passed. #156 is the sole open milestone issue; the final release commit and verified publication are next.

- `2026-09-23`: The build-only correction passed focused hosted Windows/macOS gates at 91949f9, including 30 transport/build/recovery tests and the native suite. The Linux focused run passed all 30 but its first native window fixture reached the 10-second pre-manifest startup limit (4/5 native tests passed), repeating the earlier cold-host symptom. The test now records startup elapsed time and explicit opt-in sample startup phases and waits at most 30 seconds inside the existing 90-second test deadline; native operation deadlines and production assemblies are unchanged. A fresh Linux native run is required. The ongoing full Debug/exact-version Release gates at 91949f9 remain applicable to the unchanged production binaries.

- `2026-09-23`: A Linux recheck exposed a separate scenario-build hang: an exited build's persistent MSBuild node retained both output pipes, confirmed by matching its `/proc` descriptors to the waiting test host. Stopping that exact test-owned node released the diagnostic run. Scenario builds now use the same `--disable-build-servers` policy as PreviewHost; clean Windows lifecycle/recovery tests passed 18/18 and Linux X11/Wayland/recovery/lifecycle tests passed 29/29 in 18.5 seconds without intervention. The original hosted X11 startup failure in CI 35832938516 did not reproduce in five independent recovery runs; its assertion now includes structured environment diagnostics. Validation will combine the broad platform CI at ca0c75d with focused hosted build/recovery/native gates on all three platforms for this build-only correction, plus fresh full Debug/Release and exact-version package gates. #156 remains active; #153/#155 await those final platform results.

- `2026-09-23`: Refreshed local v1.5.0 gate passed at ca0c75d: Debug and exact-version Release each passed 743 tests with 5 separately gated native-only skips, and both builds had 0 warnings/errors. All 13 release-manifest entries, provider dependency inventory/hash, Windows/Linux installers, packaged doctor/preview/lifecycle/complex CLI and MCP, standalone positive/disabled/invalid/shutdown cases, and both publish dry-runs passed. Actual Windows native tests passed 5/5 with the exact 1.5.0 provider (17af007497ec4aa883ed16eabc0aa49b); fresh Linux CLI/MCP/provider Wayland evidence is release-1.5.0-atd4phke. Hosted macOS focused run 35832935493 passed transport/session-control 12/12 and native 5/5, independently confirmed from downloaded evidence. Combined CI 35832938516 remains pending. #153/#155 remain in review; #156 is the sole active issue, and publication remains gated on that final hosted result.

- `2026-09-23`: #156 caught and reproduced a Unix IPC listener lifetime race from macOS CI 35828595250. A fast control refusal could dispose the last NamedPipeServerStream before the next acceptor was created, dropping queued clients. The server now creates and tracks the next acceptor before dispatching the connected request and disposes the pending acceptor during shutdown. A 512-request regression failed on Linux before the change and passed five consecutive runs afterward; focused bridge/session-control tests passed 12/12 on Windows and Linux. The short hosted native workflow now includes these transport tests and propagates child-process failures. Final combined CI and refreshed full Debug/exact-version Release gates are required before publication.

- `2026-09-23`: Refreshed v1.5.0 local release gate passed through 496dfc0. Full Debug and exact-version Release each passed 742 tests with 5 separately gated native-only skips, with 0 build warnings/errors. The isolated 1.5.0 checkout regenerated and verified 13 manifest entries; Windows/Linux installers, packaged doctor/preview/lifecycle/complex CLI and MCP, external-provider positive/negative lifecycle checks, actual native Windows 5/5, and both publish dry-runs passed. Fresh cross-published Linux CLI/MCP/provider native Wayland evidence is release-1.5.0-alf2hnk8. Hosted focused native Windows/macOS passed 5/5; Linux passed 5/5 on unchanged attempt 2 after an isolated pre-manifest host-startup timeout. Combined CI 35828595250 passed Windows/Linux but exposed the macOS IPC race recorded above. #153/#155 stay in review and #156 is the sole active issue; master remains 1.4.2 until the final Release 1.5.0 metadata commit. No foreign-framework interoperability or publication.

- `2026-09-23`: #156 remains active after corrected CI 35825996497 correctly stopped on a native Windows picker observation: timeoutMs=0 inherited only 100 ms for reaching the UI dispatcher. Zero-wait detect now keeps its single-probe semantics with a bounded 3000 ms scheduling allowance; action deadlines and caller cancellation remain intact. A deterministic 250 ms queued-UI regression failed before the fix and the full ExplicitInputTests group passed 5/5 afterward. A freshly packaged provider passed actual Windows native tests 5/5 (131fe46039444495918620ee0492c3d5). Next: short hosted native platform gates, final combined CI, fresh full Debug/Release validation (742 tests plus 5 native skips expected) and regenerated exact 1.5.0 artifacts before publication.

- `2026-09-23`: Release tracker #156 remains active. Evidence downloaded from final CI 35823782385 exposed a masked Windows native-dialog failure (4/5): a later successful command hid the child PowerShell exit. The affected multi-command CI/release steps now stop on native failures; a deliberate exit-23 probe verifies no later command runs. The native fixture waits for cold dialog readiness for at most 10 seconds without repeating its open action, then performs the unchanged bounded MCP operations. Release build passed with 0 warnings/errors and all 5 native Windows tests passed locally (5236ce06d22a4a668aafc3704834da48). A fresh combined hosted gate is required; the earlier green Windows status is insufficient. Product binaries are unchanged from a692aa7, so prior full Debug/Release (741 passed plus 5 separately gated native skips each), exact 1.5.0 packaging and publish dry-runs remain relevant. No release commit/tag/publication yet.

- `2026-09-23`: All 38 v1.5.0 implementation slices are committed (#117-#153 and #155); #154 remains cancelled. #117-#152 are closed after combined Windows/Linux/native macOS CI 35821036688 at f6b6c84. #153 is bc48c3e (real Windows UIA/Linux AT-SPI CLI/MCP audit; both native 5/5); #155 is b231af4 (private Weston 13 native Wayland; Linux 31/31 environment/profile/recovery tests and real CLI/MCP text/capture at 1x/2x, failure/cleanup gates). #153/#155 await final combined CI 35823782385 at a692aa7 on temporary ci/v1.5.0-final. #156 is active. Full local Debug and Release each passed 741 tests with 5 separately gated native-only skips. Exact 1.5.0 artifact checks passed: 13 manifest entries, Windows/Linux installers, packaged doctor/preview/lifecycle/complex CLI+MCP, standalone provider positives/negatives, both publish dry-runs and cross-published Linux CLI/MCP/provider on native Wayland (release-1.5.0-mxy8puzd). Artifacts are in isolated worktree C:\Users\soosr\.codex\worktrees\avascope-release-1-5\AvaScope; master remains 1.4.2. Runtime/sample Avalonia 12.1.3; standalone Bridge baseline 12.1.0. No foreign framework interoperability; unsupported native/Wayland combinations remain explicit.
...
- `2026-09-03`: Implemented #112 in `bb7035a` and moved it to review pending the combined release validation. The bridge now continues accepting named-pipe clients while four bounded handlers process accepted connections, tracks all active pipes for shutdown, and has a deterministic stalled-request plus 48-request mixed-concurrency regression. Started #113 for captured-target synthetic gesture lifecycle correctness; release tracker #114 remains ready.
- `2026-09-03`: Published `v1.4.1` from exact release commit `37810fc0e02d0be7203eb9c50e4beeb3c723cf86` through successful Release workflow `33724707621`. The remote tag matches; the public, non-prerelease GitHub Release has 12/12 assets; and fresh downloads verified all 11 manifest entries by exact name, size, and SHA-256 plus every package metadata version. Nuget.org exposes Protocol/Core/Bridge `1.4.1` as latest; all three CDN packages pass repository-signature verification and their unsigned entries match the release packages. GitHub Packages publication succeeded for all three packages. Release tracker #111 and milestone `v1.4.1` are closed.
...
- `2026-09-03`: Completed #107 in `d28aebe`, `5aeec24`, `0bcd625`, `18db34b`, and `6135a5f`. Added the optional `RuntimeEvidencePolicy` boundary for text/AutomationId/excluded-control redaction, screenshot masking, marker-owned retention, pre-serialization local audit and process-output redaction, action/custom-action/gesture/destructive gates, live local session/PID authorization, sanitized idempotency records, and unavailable network upload. Unsafe paths, mismatched ownership, non-empty unowned runs, and reparse traversal inside the owned root fail closed while macOS system ancestors such as `/var` remain compatible. Debug and Release builds completed with `0` warnings/errors and all `500` tests passed in both configurations. Hosted CI `33695657065` passed Windows Build/Test/Pack, packaged lifecycle, installer and artifact verification; Linux packaged lifecycle and installer validation; and macOS all `500` tests, native Retina runtime/preview validation, packaged lifecycle, native artifact and installer validation. Started #108 for repeatable release-shaped complex workflow validation.
- `2026-09-02`: Completed #106 in `1f3b5da`, `2c4f309`, `14e358e`, and `3abb982`. One `run-scenario` request can now perform an explicit or implicit structured project build, directly launch either a tokenized command or the built project assembly, wait for the exact launched process's bridge manifest and registered top levels, attach, execute the semantic workflow, preserve build/launch logs and stage-specific failure evidence, and optionally close the session plus terminate only the identity-verified owned process tree. Readiness IPC inherits the bounded scenario deadline instead of the client's shorter default; a deterministic 5.5-second first-response regression covers loaded runners, and the test bridge uses a short Unix-safe pipe name. Normal JSON and timeline evidence exposes environment-variable names and argument counts without copying values. Debug and Release builds completed with `0` warnings/errors and all `477` tests passed in both configurations; packaged win-x64 lifecycle and PowerShell validation passed locally. Hosted CI `33685175325` passed Windows Build/Test/Pack, packaged lifecycle, installer and artifact verification; Linux packaged lifecycle and installer validation; and macOS all `477` tests, native runtime/preview validation, packaged lifecycle, and native artifact validation. Started #107 for fail-closed evidence redaction, retention, and action safety policy.
- `2026-09-02`: Completed #105 in `111e5e0`. Semantic workflow actions can now capture bounded pre-state, execute once under the existing idempotency contract, wait on the shared typed condition model, and return explicit passed, failed, timed-out, or partial verification evidence. Requested failure packs retain available inspection state, screenshots, bounded tree fragments, selector candidates, active top levels, adjacent workflow context, and unavailable markers when individual artifacts fail. Stable execution directories export agreeing JSON, Markdown, and JUnit reports for CLI, MCP, workflow, and attached-scenario paths; response budgeting preserves artifact references and a compact agent-review summary. Focused coverage passed `18` tests; full Debug and Release builds completed with `0` warnings/errors and all `463` tests passed in both configurations. Hosted CI `33674831016` attempt 2 passed Windows Build/Test/Pack and installer/artifact verification, Linux installer validation, and macOS full tests plus native runtime/preview/artifact validation after the unchanged rerun cleared an unrelated transient pointer-diagnostics cleanup file lock. Started #106 for bounded build-launch-attach lifecycle orchestration and exact owned-process cleanup.
- `2026-09-02`: Completed #104 in `3247df9`. Workflow requests now compile before Bridge dispatch or artifact-directory creation and support typed `if`/`else`, leaf `optional`, bounded `retry_until`, `${variable}` substitution, and reusable parameterized acyclic fragments. Side-effecting retry bodies require path-scoped idempotency, and retries re-evaluate the shared typed wait state without duplicating dispatch. `validateOnly` returns a bounded expanded plan and all collected static diagnostics; invalid scenarios fail before launch/attach. Results and Markdown timelines expose execution path, parent step, attempt, source fragment, and explicit skipped/retried outcomes. Fixed nesting, expansion, execution, retry, artifact, variable, fragment, and total-time limits are advertised through capabilities and enforced before execution; shared response budgeting preserves complete oversized plans in local artifacts. Debug and Release builds completed with `0` warnings/errors and all `455` tests passed in both configurations. Hosted CI `33669183352` attempt 2 passed Windows Build/Test/Pack and artifact verification, Linux installer validation, and macOS full tests plus native runtime/preview/artifact validation after the unchanged rerun cleared a transient named-pipe harness timeout. Started #105 for observe-act-verify and automatic runtime failure evidence packs.
- `2026-09-02`: Completed #103 in `841ddee`. Semantic workflows and runtime scenarios can declare workflow-scoped top-level aliases with exact title, kind, active-state, and optional session selectors; every aliased step, typed wait, assertion, screenshot, and automatic evidence capture re-resolves the active top-level immediately before use. Closed and reopened windows may receive new runtime ids without invalidating the workflow, while unknown, missing, ambiguous, and cross-session aliases return bounded candidates and remediation details. Results and scenario timelines retain the logical alias plus the resolved runtime id as diagnostic evidence. Protocol, Core, CLI, MCP schema/capability, documentation, idempotent replay, and real two-window headless coverage are included. Debug and Release builds passed with `0` warnings/errors and all `448` tests passed in both configurations. Hosted CI `33663497807` passed Windows Build/Test/Pack and installer/artifact verification, Linux installer validation, and macOS full tests plus native runtime/preview/artifact validation. Started #104 for bounded conditional, retryable, variable-driven, and reusable workflow composition.
- `2026-09-02`: Completed #102 in `35d1b20`. Added optional typed `waitCondition` inputs and structured `waitObservation` output while preserving `assertProperty`/`expected`; per-poll actionable selector re-resolution; deterministic existence/disappearance, visible/hidden, enabled/disabled, checked/unchecked, selected/text/value, rendered-bounds, command-executability, binding-value, top-level open/close, and baseline-change waits; invariant typed comparisons; and distinct unavailable versus false-condition timeout evidence with bounded last candidates, elapsed time, and next action. Bridge inspection now exposes the required public Avalonia 12 text, toggle, selection, range, and command state. Protocol, real headless Bridge/Core, CLI pipe, live MCP schema, capability, and documentation coverage passed; Debug and Release builds completed with `0` warnings/errors and all `445` tests passed in both configurations. Hosted CI `33658851376` passed Windows Build/Test/Pack and installer/artifact verification, Linux installer validation, and macOS full tests plus native runtime/preview/artifact validation. Started #103 for workflow-scoped top-level aliases and resilient multi-window routing.
- `2026-09-02`: Completed #101 in `747117b`. Runtime tree/search/inspect results now publish shared visible/enabled/rendered/actionable state and available actions; CLI, MCP, waits, validations, binding/command selectors, and semantic actions use the same optional state filters. Workflow input carries generation context into the Bridge UI-thread dispatch boundary, re-resolves once only after a proven pre-dispatch stale result, and never retries a possible post-dispatch side effect. Ambiguity diagnostics include bounded identity/state/bounds/top-level/action candidates, while raw node ids remain diagnostic evidence. Headless coverage verifies hidden, disabled, zero-bounds, clipped, and non-interactive exclusion, template/content recreation, stale recovery, no post-dispatch retry, idempotent replay, and CLI/MCP parity. Debug and Release builds passed with `0` warnings/errors and all `442` tests passed in both configurations. Hosted CI `33653895062` passed Windows Build/Test/Pack and installer verification, Linux installer validation, and macOS full tests plus native runtime/preview/package validation. Started #102 for typed rendered, command, binding, selection, disappearance, and top-level lifecycle waits.
- `2026-09-02`: Completed #100 in `203c52b`. Added disabled-by-default, app-opted-in and activation-allowlisted custom actions scoped to live visual instances; parameter schemas, required-state/executability discovery, safety classifications, dual app/request authorization for destructive actions, UI-thread execution, bounded audit results, versioned IPC/session capabilities, selector-based workflows, CLI/MCP adapters, and a `confirm`/`reset` sample. Focused coverage passed `10` tests; full Debug and Release builds completed with `0` warnings/errors and all `438` tests passed in both configurations. Hosted CI `33647360533` passed Windows Build/Test/Pack and installer verification, Linux installer validation, and macOS full tests plus native runtime/preview/package validation. Started #101 for actionable selector filtering, immediate re-resolution, and safe stale-node recovery.
...
- `2026-07-30`: Hosted CI retry `30521551666` reduced macOS failures from `81` to `11`; all remaining failures were protocol tests where raw caller-provided paths or derived file URLs were incorrectly made platform-normalized by the first correction. Raw-path contracts are preserved, file URL expectations now derive from the effective full path, and all `63` protocol contract tests pass locally.
...
- `2026-07-29`: The final post-hardening v1.2.0 local release gate passed from `57c9ae7`. Debug and Release builds completed with `0` warnings/errors and all `417` tests passed in each configuration. Exact `1.2.0` Protocol/Core/Bridge NuGet packages, Windows/Linux portable ZIPs, Windows/Linux installers, and the seven-entry SHA-256 manifest passed verification. Packaged Windows install/repair/doctor/MCP/uninstall, packaged doctor and real preview, WSL Ubuntu Linux installer smoke with temporary .NET `10.0.10`, intake privacy validation, capability/version smoke, and NuGet/GitHub Release dry-runs passed. Tag and release `v1.2.0` remain absent before publication.
...
- Status: `Done` (tracker and milestone closed)
- Owner: autonomous agent
- Result: all accepted slices validated; v1.5.0 published with independent remote tag, asset and package verification.

## Next Action

No v1.5.0 work remains. Define the next release scope through GitHub before starting further implementation. #154 remains cancelled; no foreign-framework interoperability is in scope.

## Latest Validation

- `2026-09-23`: v1.5.0 publication passed at `f5aaa8651e77b14f18982e662c4d23a347206bfb` in Release workflow `35838400133`: 743 tests plus five separately gated native-only skips, artifact/installer validation and six package pushes. Fresh remote downloads passed 14-asset/13-manifest checks, standalone-provider verification and three `dotnet nuget verify --all` checks; all unsigned NuGet entries match the GitHub Release packages. Earlier full Debug/Release, broad platform CI `35832938516`, focused Windows/macOS/Linux gates `35836813288`/`35836819598`/`35837679713`, packaged CLI/MCP and native Wayland checks passed as recorded above. Local final evidence is `.git/agent-work/v1.5.0/remote-release-verification.log` and `remote-verification-ca9ed0863223452b9616eb29863ddf78/verification.json`.
...
- `2026-07-28`: Started open-source readiness tracker #73. Official Gitleaks `v8.30.1` was checksum-verified and scanned all `239` commits plus the current worktree with `0` findings. A separate tracked-file audit found no credential-like filenames, absolute local user paths, or common token/private-key patterns. The existing public Git author email was explicitly accepted; agent-development instructions and planning history remain intentionally public.
...
- `2026-07-28`: Limited verification-terminal color to the status marker: all output uses white console text, while only `SUCCESS` is green or `FAILED` is red through reset-bounded ANSI sequences. Artifact-backed tests assert the expected green/red sequence and reject the opposite color for both success and simulated failure paths. The setup rebuilt successfully; all seven release artifacts were verified; Release build passed with 0 warnings/errors; focused installer/stable-surface tests passed (`7`); and `git diff --check` passed with line-ending normalization warnings only. No version bump, tag, or release was created.
- `2026-07-28`: Reopened #70 for final verification-terminal color polish. The console remains white and only the `SUCCESS` or `FAILED` status marker receives green or red ANSI color. No version bump, tag, or release publication is in scope.
- `2026-07-28`: Restored the Windows Finish-page verification checkbox and replaced the disappearing raw command terminal with a dedicated persistent ASCII status screen. The terminal displays the installed version, command/MCP hints, green `SUCCESS` or red `FAILED`, actionable .NET 10 guidance on failure, and waits for a keypress before closing. Artifact-backed tests now exercise and assert both success and simulated failure screens in addition to install/doctor/MCP/repair/uninstall. The setup rebuilt successfully; all seven release artifacts were verified; Release build passed with 0 warnings/errors; focused installer/stable-surface tests passed (`7`); and `git diff --check` passed with line-ending normalization warnings only. No version bump, tag, or release was created.
- `2026-07-28`: Reopened #70 after clarification that the Finish-page verification checkbox was the desired interaction; only the disappearing, uninformative terminal was defective. The corrective design restores the checkbox and replaces raw `--version` output with a persistent ASCII `SUCCESS`/`FAILED` status screen that closes only after a keypress. No version bump, tag, or release publication is in scope.
- `2026-07-28`: Removed the Windows Finish-page `Verify the AvaScope command` action that flashed a terminal. Setup now runs `avascope.exe --version` automatically with captured hidden output, requires the exact packaged version, reports an actionable .NET 10 runtime error on failure, and shows explicit verified-success guidance on the final wizard page. The rebuilt Windows install/doctor/MCP/repair/uninstall smoke passed; all seven release artifacts were verified; Release build passed with 0 warnings/errors; focused installer/stable-surface tests passed (`7`); and `git diff --check` passed with line-ending normalization warnings only. No version bump, tag, or release was created.
- `2026-07-28`: Reopened #70 for a second hands-on Windows installer UX correction. The optional Finish-page verification action only flashed a terminal, so it is being replaced by automatic hidden CLI startup verification with explicit wizard success/failure messaging. No version bump, tag, or release publication is in scope.
- `2026-07-28`: Replaced the Windows console bootstrapper with a real Inno Setup `modern dynamic` wizard producing `AvaScopeSetup.exe`. The per-user wizard includes license, destination, optional PATH, deterministic repair/upgrade replacement, discovery metadata, and Apps & Features uninstall; the Linux terminal installer remains unchanged. The complete local Release gate passed with 0 warnings/errors and `371` tests, seven verified hashed artifacts, installed Windows CLI `--version`/`doctor`/MCP/repair/uninstall smoke, packaged sample preview, and WSL Ubuntu Linux installer smoke using a temporary user-local .NET 10 runtime. Focused post-gate contract/installer tests passed (`7`); NuGet and GitHub Release dry-runs passed without publication; `git diff --check` passed with line-ending normalization warnings only. Version remains `1.1.2`; no tag or release was created.
- `2026-07-28`: Reopened #70 after hands-on review found that the Windows artifact was a console bootstrapper rather than the expected graphical installer. The issue and roadmap card returned to `In Progress / 25% / Current Slice`. The corrective slice replaces only the Windows packaging surface with a non-admin Inno Setup wizard while retaining the validated Linux terminal installer. No version bump, tag, or release publication is in scope.
- `2026-07-28`: Completed local implementation for #70 in commit `664e17a` and opened draft PR #71. Added a framework-dependent single-file installer host and generated `avascope-win-x64-installer.exe` plus `avascope-linux-x64-installer` from the same multi-file payloads as the portable ZIPs. Implemented non-admin install, deterministic repair/upgrade replacement, discovery metadata, legal-file installation, owned-path-safe uninstall, Windows current-user PATH/Apps & Features integration, Linux `~/.local/bin` setup, and an optional Authenticode signing hook. Added Windows/Linux CI lanes and artifact-backed tests for verify, install, `--version`, `doctor`, MCP stdio/startup, repair, unsafe-uninstall rejection, and uninstall. Debug and Release builds passed with 0 warnings/errors; full Debug and Release tests each passed `371`; Windows and WSL Ubuntu installer smokes passed; the local release workflow verified seven hashed artifacts; NuGet and GitHub Release dry-runs passed without publication. Version remains `1.1.2`; no tag or release was created.
...
- `2026-07-02`: Completed and closed GitHub issue #59 in commit `5ceaa4ff6e0c82202821de00a89b2426a07c68ed`; pushed to `master`, issue moved to `status:done`, and roadmap card moved to `Done / 100% / Completed`. Started GitHub issue #60 for the `v1.1.1` patch milestone. The issue is `status:in-progress`, the roadmap card is `In Progress / 25% / Current Slice`, and intended validation is focused CLI help/argument-validation tests proving `baseline-create` explicitly rejects run-index flags with guidance, capability metadata/stable-surface checks for run-index support accuracy, build, relevant CLI tests, full Debug tests, and `git diff --check`.
...
- `2026-07-01`: Completed GitHub issue #46 by adding bounded runtime pointer-path diagnostics for hover, popup, tooltip, and pointer transition workflows. Added protocol request/response DTOs, Core `RuntimePointerDiagnosticsRunner`, CLI `pointer-diagnostics`, MCP `pointer_diagnostics`, capability discovery, stable-surface docs, user/agent docs, screenshot pointer-overlay artifacts, visual-tree hit-path/layer inference across available top-levels, assert-hit steps, and parent hover exit warnings with explicit `bounds_snapshot_inference` provenance. Validation passed with `dotnet build AvaScope.slnx --no-restore -v:minimal`, focused #46/capability/MCP tests (`6` passed), focused docs tests (`6` passed), full Debug tests (`336` passed), and `git diff --check` with only LF/CRLF normalization warnings.
- `2026-07-01`: Completed GitHub issue #49 by adding generated HTML report click-to-node provenance mapping to runtime mutation evidence artifacts. The evidence HTML now reads captured before/after visual-tree snapshots, embeds bounded screenshot node maps, maps clicks on before/after screenshots to the smallest containing or nearest bounded node, and shows available source/property/binding provenance without changing the stable response DTOs. Added exporter regression coverage and updated agent/user docs. Validation passed with `dotnet build AvaScope.slnx --no-restore -v:minimal`, focused exporter/source/protocol tests (`54` passed), focused docs tests (`6` passed), full Debug tests (`333` passed), and `git diff --check` with only LF/CRLF normalization warnings.
