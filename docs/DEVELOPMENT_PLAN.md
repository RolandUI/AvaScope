...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117-#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117-#134 are closed; CI 35772545523 passed Windows/Linux/macOS. #135 adds 11 live-schema-checked recipes, clean-project onboarding, repeatable CLI/MCP multi-window/environment/application conformance, and separate actual-agent measurement. Windows focused tests passed (11), Linux focused tests passed (18), and native Linux X11 conformance passed all 12 repeated cases plus onboarding (94 public calls). Windows Headless MCP passed six repeated cases plus onboarding (56 calls); six separate agent-directed Headless trials completed with correct classifications, 23 measured MCP calls and no retries. Client/model uncertainty and open-label fixture provenance are explicit; reports remain under artifacts/agent-evaluations (agent-directed-e4983858e9da438ea7b07a8d63b7e95c for measured trials). Fresh package/standalone enabled/disabled integration passes. Await the combined three-platform CI before closing #135. Product version stays `1.4.2` until the final validated release commit.
