...
Start GitHub issue #14 (`R0.7.0-M6 Release Candidate And Version Bump`). Before release work, move it from `status:backlog` to `status:in-progress`, add a start comment, set `docs/RELEASE_PLAN.md` to Release Candidate, run the release gate from `docs/VALIDATION.md`, bump `Directory.Build.props` to `0.7.0` in commit `Release 0.7.0`, and verify the GitHub Release workflow publishes `v0.7.0`.
