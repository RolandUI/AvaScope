...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117�#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117�#131 are closed. Combined CI 35757152151 passed exact 67f8ab1 on Windows/Linux/macOS after an unchanged retry of an upstream Avalonia HeadlessUnitTestSession.Dispose race; 576 tests and all native/package gates passed, plus 22 focused WSL tests. Active #132 adds explicit semantic/synthetic/native input, compound gestures/keys, owned native dialogs and a reflection-only predefined picker hook. 88 focused tests passed; real Windows and managed X11 CLI/MCP native input, interruption cleanup and GTK/Win32 picker gates passed. macOS native drag is explicitly unsupported because Avalonia reads global held-button state; the native CI gate validates this refusal plus explicit synthetic drag. Three-platform CI is pending for #132. Product version stays `1.4.2` until the final validated release commit.
