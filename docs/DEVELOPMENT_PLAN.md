...
- `2026-09-22`: Active release `v1.5.0` / tracker #156 implements all #117–#155. #117 runtime foundation was pushed in `58eeb05`; #119 adds a copyable BCL-only loader and compile-time-gated standalone host with no AvaScope assembly references, verified dependency resolution, pins and structured failures. Release builds of the test project and diagnostics-enabled sample passed with `0` warnings/errors. #117/#119 remain open in review pending the combined standalone gate; next active slice #121 supplies provider packaging/metadata and shared validation. Tests have intentionally not run per edit. Current committed product version remains `1.4.2`, Avalonia `12.1.0`, `net10.0`; final full local/hosted/package validation is required before publication.
