...
- `2026-09-24`: #175 is the sole active issue; #172 is in review. Moving DataGrid theme registration before XAML fixes the reproduced null template: pre-fix regression fails, all 19 fixture/table checks pass afterward. Native `expanded-direct-03` now renders headers/rows and selects offscreen QA-175 (T007/T008); before/after images are retained. The expanded headless lifecycle check is running with rendered-row/realized-cell assertions; its first attempt had a missing treeKind in the new harness request, corrected by reusing the returned target. Separate product #176 now queues T009: BeginEdit makes the previously complete 32-deep selector hit depth_limit and leaves a draft. T010 confirms 937 visited nodes; original data remains unchanged. Escape cancelled the cell but the guarded key sequence stopped after target detachment; row edit still needs explicit cancellation. Preserve these failures and inspect state before further edits. Prior native CI `36047877819` passes all six old lab paths on `6cd33b2`; it does not validate the #175 fix/new assertions. Continue #175 native/standalone validation, then #176 and remaining #172/#173/#174 work. No release.
