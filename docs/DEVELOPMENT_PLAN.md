...
- Goal: complete the guarded release-candidate gate, publish the exact `Release 1.4.0` commit, and verify every remote package and artifact.

## Next Action

Commit and push only the staged version/readiness metadata as `Release 1.4.0`, then verify the published tag, packages, assets, sizes, and SHA-256 digests.
