...
- `2026-09-24`: #166 is active for final campaign coverage. Actual isolated preview P001–P015 passes on 12.1.3: 1×/2× text, light/dark, en-US/hu-HU, template edits, failed-XAML state and recovery, exact CLI/MCP PNG parity, bounded watch and closed-session refusal/idempotence. The MCP scenario client preserves the explicit preview store; the record was verified only in its owned directory. Browser review of exported viewer HTML is blocked by the tool's file-URL policy; PNGs were viewed independently. #170 `678ac0b` passes full Release 802 tests plus five explicit native skips and focused Debug 17/17. #157 `c8b7dc3` passes full Debug/Release 801 each and all six hosted lab combinations; CI `36019901130` has only macOS packaging still running. Final combined/native coverage remains. Native Retina is unverified; no publication.
