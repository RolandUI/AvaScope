...
- `2026-09-03`: Started priority patch target `v1.4.2` with #112 in progress, #113 ready, and release tracker #114 ready. Scope is locked to concurrent same-session Bridge IPC response reliability and captured-target synthetic drag/swipe lifecycle cleanup. The two fixes will be validated together in one consolidated local full Release pass and one combined hosted Windows/Linux/native-macOS CI run before packaging and publication.
