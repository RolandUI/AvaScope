...
- `2026-09-25`: #184 remains sole active at 8bebfe6. Full before-fix CI 36096369354 continues: Windows Build/Test/Pack passed (855 pass/six explicit skips, 11m34s, retained logs), macOS native QA passed, Unix reproduction/full gate pending. Native ownership-direct-01 passes 24 checks; fresh clean a099b86 ownership-standalone-01 passes 34, each through 25 MCP/seven CLI calls. Both verify competing acquire/input/cleanup refusal, owner input once, explicit handoff/renewal, stale/invalid/expired refusal, reacquire/reset and only-owned/repeated cleanup. Viewed Windows/rendered images and journal agree; token-redacted evidence has no known raw lease token. Direct app 19164/client 6468 and standalone app 1760/client 15016 are gone; clients exit 0/empty stderr, Stop confirms already_exited. This Windows/12.1.3/Win32/1x evidence does not prove the Unix fixture cause. Local recovery remains ten pass/one explicit Unix skip; no listener or retry fix yet. Await hosted reproduction; #185/#174 and combined gates remain. No local QA/test process and no release.
