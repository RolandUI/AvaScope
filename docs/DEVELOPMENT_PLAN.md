...
- `2026-09-24`: #171's selected CI `36031948678` (`a6783cc`) passes the entire Windows job and all three native lab jobs. Downloaded Windows TRX verifies 803 passed and five explicit native skips: the original Unicode case takes 0.636 s, new deadline/recovery case 5.054 s; logs agree, build has zero warnings/errors. Artifact retention is verified. Six native clean-source identities, one-toggle/one-edit/case-distinct snapshots and owned termination also agree (`instrumented-hosted-lab/verified-all-platforms.json`). Mac remains actual 1×, Full HD intent clamped to 1920×649 DIP. Downstream macOS job `107752417815` and Linux job `107752417852` are now running; continue this exact run, then inspect their downloaded evidence. No local process is running. Original timeout cause and native Retina acceptance remain unresolved.
