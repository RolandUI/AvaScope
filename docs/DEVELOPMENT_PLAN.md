...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117–#155. The #117/#119/#121 standalone foundation is implemented and ready for hosted validation: explicit bootstrap, automatic lifetime registration, BCL-only loader, 35-file provider ZIP/checksum, exact pins, shared-assembly identity metadata, CLI/MCP provider verification, release packaging and platform CI gates. External-host validation passed on Windows headless (`34b37e240ed7433ab813453632d56048`) and native Windows (`87a193106ab5422a9be2536435c0d742`), covering real CLI/MCP, windows, screenshots, disabled/load-only, corrupt/incompatible/missing providers, invalid DLL, missing bootstrap, normal exit and remote cleanup. The 164-test bridge/provider/discovery batch passed 163 with one upstream Headless session-disposal race; its unchanged screenshot regression plus bootstrap/provider tests then passed `17/17`. All eight follow-up contract/duplicate-loader checks are covered by seven initial passes and the corrected additive tool-list rerun (`1/1`). Hosted Windows passed all 521 tests after the stable-surface documentation correction. Its packaging gate exposed Windows PowerShell 5 ZIP backslash entries; portable slash entries now pass local PowerShell 5 packaging/inventory/checksum verification. Provider fixture builds use isolated artifact paths. Hosted platform evidence remains pending. #118/#120 onboarding is implemented: read-only file/line/hash guidance plus staged build/provider/activation/discovery/readiness/tree/inspection/screenshot/focus/cleanup verification, including independent production-disabled checks. The batched 51 tests had one stage-classification failure; after fixing it all 16 affected verifier/contract tests pass. All eight generated package/standalone, enabled/disabled, CLI/MCP runs passed on native Windows (`8d84fb81fdbf44e9ab9d362875514f71`); hosted gates are next. #122 shared test profiles are implemented and the solution build passes with zero warnings: deterministic platform merge, relative paths, unique runs, pinned providers, environment references/redacted views and existing CLI/MCP run-scenario integration. Schema/redaction and real profile gates are batched with the next active slice, #123 target/platform doctor. Hosted onboarding CI run: 35734957563. Product version remains `1.4.2` until the final validated `Release 1.5.0` commit; no accepted feature has been deferred.
