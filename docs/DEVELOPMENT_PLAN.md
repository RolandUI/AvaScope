...
- `R0.7.0-M5 CLI/MCP Runtime Experiment Review`
- GitHub Issue: https://github.com/RolandUI/AvaScope/issues/13
- GitHub Milestone: `v0.7.0`
- Status: `Ready`
- Owner: unassigned
- Started: pending
- Goal: give agents a concise review surface for runtime experiments so they can inspect active overrides, understand evidence, and decide the next action.

## Next Action

Start GitHub issue #13 (`R0.7.0-M5 CLI/MCP Runtime Experiment Review`). Before implementation, move it from `status:ready` to `status:in-progress`, add a start comment, and keep the work scoped to mutation history, active override summaries, reset handoff, and local review artifact paths.
