...
- `R0.7.0-M4 Agent Session Safety And Reset Semantics`
- GitHub Issue: https://github.com/RolandUI/AvaScope/issues/12
- GitHub Milestone: `v0.7.0`
- Status: `Ready`
- Owner: unassigned
- Started: pending
- Goal: keep runtime mutation opt-in, local-only, reversible, auditable, and easy to clean up across attach, reset, close, and app-exit flows.

## Next Action

Start GitHub issue #12 (`R0.7.0-M4 Agent Session Safety And Reset Semantics`). Before implementation, move it from `status:ready` to `status:in-progress`, add a start comment, and keep the work scoped to capability discovery, session/top-level mutation scoping, reset/close cleanup, stale target diagnostics, and safety documentation.
