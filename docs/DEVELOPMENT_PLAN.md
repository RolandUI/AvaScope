...
- `2026-09-22`: Started the user-authorized GOAL MODE implementation of #117–#155 followed by release. Milestone `v1.5.0` and tracker #156 lock all 39 issues. #117 runtime foundation now provides explicit stable reflection bootstrap, pre-activation compatibility/lifetime checks, automatic window registration, and shutdown/remote-close cleanup; failed activation no longer leaves `Current` active. Release build of the test project and dependencies passed with `0` warnings/errors. Tests are added but intentionally await the combined #117/#119/#121 standalone foundation gate; #117 stays open in review. Next active slice: #119 loader sample, then #121 provider distribution/metadata. Current committed product version remains `1.4.2`, Avalonia `12.1.0`, `net10.0`; final full local/hosted/package validation is required before publication.
