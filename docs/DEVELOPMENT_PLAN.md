...
- `2026-09-25`: #184 is the sole active issue after #190 review at b70e35c/c37abd3. A bounded first-response gate and actual-fixture queued-acquire regression now exercise the candidate Unix listener-disposal race; existing outsider failure diagnostics redact the token and retain bounded message/allowlisted IPC details. Fixture build is clean; Windows recovery run passes 10 cases/1 explicit Unix skip (1m32s, clean build). Local WSL/Linux is unavailable. The official .NET 10 source supports the hypothesis, not the original hosted cause; no listener fix or retry change yet. Push this diagnostic checkpoint for hosted before/fix/after validation. #185/#174 and combined gates remain; no local QA/test process and no release.
