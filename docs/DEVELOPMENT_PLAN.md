...
Publish the validated scope with the exact `Release 1.5.0` metadata commit, verify the remote tag, all 14 release assets/13 manifest checksums and all three NuGet packages, then close #156 and milestone v1.5.0 and record the release handoff. #154 remains cancelled; no foreign-framework interoperability is in scope.
