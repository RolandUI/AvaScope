...
- Started: `2026-07-02`
- Goal: clarify that `baseline-create` does not support run-index flags while keeping capability metadata accurate for commands that do.

## Next Action

Implement GitHub issue #60 with focused CLI help/argument-validation tests proving `baseline-create` explicitly rejects run-index flags with guidance and capability metadata continues to avoid advertising run-index support for `baseline-create`; then validate, commit, push, update the issue, and continue through the remaining `v1.1.1` smoke-test bugs.
