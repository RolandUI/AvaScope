...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117–#153 and #155 (38 issues); #154 cancelled by user: no foreign framework interoperability/adapters/import. #117–#123 are closed after hosted run 35738762089 passed Windows, Linux and native macOS. #124 is implemented in 65e154b; managed/existing authenticated local X11 passed native WSL environment tests and external-provider CLI/MCP scenarios. Active #126 adds observed backend support to session/top-level capabilities and actual input/screenshot provenance, including dry-run/no-hit and gesture fallback, preserved through workflow evidence. Local Release bridge/protocol/evidence batch passed 127/127; native X11 provider CLI/MCP route assertions and cleanup also passed (324492ada390441cb596fe121d3092ac, zero-warning Linux build). The #125 consolidated platform matrix is next. Product version remains `1.4.2` until the final validated `Release 1.5.0` commit.
