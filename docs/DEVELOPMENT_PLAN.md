...
- `2026-09-24`: Resumed with explicitly expanded real-app coverage. #172 is the sole in-progress issue: shared labeled forms, editable keyed DataGrid and richer menu/pointer/keyboard input, with independent journal/reset checks in Direct and standalone hosts. #173 follows for declared scenes/operations/diagnostics; #174 tracks the complete source-derived capability matrix and fresh exploratory campaign. #166 returns to review as the umbrella. Previous validated gates remain historical evidence, not validation of new fixture code. Preserve the native Retina, HTML-viewer and original #171 timeout limitations. No release, version bump or publication.
