...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117–#153 and #155 (38 issues). User cancelled #154: no Python/Robot adapters, PlatynUI interoperability or foreign workflow import. #117–#123 are implemented/in review; native Windows 12-run onboarding/profile gate and target doctor passed. Updated hosted run 35738762089 has Windows/Linux success and macOS packaging validation still running. #124 managed/existing Linux X11 environments are implemented in the existing scenario/profile engine: authenticated per-run Xvfb, Linux abstract Unix sockets/no TCP, optional Openbox/private D-Bus, readiness, bounded redacted logs and exact helper cleanup. Windows profile/doctor/scenario batch passed 28/28; native Linux WSL build passed with zero warnings, all 5 environment tests passed (parallel/auth-denial/no-TCP/existing/crash/cancellation), and external-provider CLI/MCP scenarios plus missing-prerequisite failure passed (`0fa87af02aea4dcf9fd70da991dcc660`). WSLg has a read-only socket directory; abstract Unix transport supports it without system changes. Local Linux toolchain is task-owned under /home/soosr/.cache/avascope-validation-91ae708; no privileged install or desktop configuration change. Next slice #126 adds actual backend/operation/screenshot provenance before the #125 consolidated native matrix. Product version remains `1.4.2` until the final validated `Release 1.5.0` commit.
