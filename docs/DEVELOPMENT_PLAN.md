...
- `2026-09-22`: Active release `v1.5.0` / #156 covers #117-#153 and #155 (38 issues); #154 cancelled: no foreign framework interoperability/adapters/import. #117-#131 are closed. #132 is in review: Windows/Linux native input/dialog gates pass; macOS validation exposed CoreGraphics loading through the provider inventory guard, corrected through c0695aa with dedicated native validation pending. Active #133 implements durable scenario records, exclusive renewable control leases, bounded reconnect/recovery and exact owned-resource cleanup. Windows full Release suite passed 586 tests (one separately gated native test skipped), followed by 40 focused tests including conflict retention and killed-agent recovery. WSL passed 28 focused tests including real Xvfb/Openbox/D-Bus retention and recovery after a lease conflict, plus managed-X11 external-provider scenarios. The native-input regression and cross-platform CI are the remaining checkpoint gates. Product version stays `1.4.2` until the final validated release commit.
