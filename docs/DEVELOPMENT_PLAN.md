...
- `2026-09-02`: Implemented #106 locally. One `run-scenario` request can now perform an explicit or implicit structured project build, directly launch either a tokenized command or the built project assembly, wait for the exact launched process's bridge manifest and registered top levels, attach, execute the semantic workflow, preserve build/launch logs and stage-specific failure evidence, and optionally close the session plus terminate only the identity-verified owned process tree. Readiness IPC inherits the bounded scenario deadline instead of the client's shorter default; a deterministic 5.5-second first-response regression covers loaded runners. Normal JSON and timeline evidence exposes environment-variable names and argument counts without copying values. Success, build/start/exit/readiness/attach/cancellation failure, idempotent cleanup, PID-reuse refusal, response-budget, CLI, MCP, protocol, and documentation coverage is included. Debug and Release builds completed with `0` warnings/errors and all `477` tests passed in both configurations; the packaged win-x64 lifecycle gate and PowerShell syntax validation passed. Hosted Windows/Linux/macOS validation is pending the pushed commit.
