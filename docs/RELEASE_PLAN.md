...
- `2026-09-02`: Implemented #103 locally with workflow-scoped semantic top-level aliases that re-resolve each step, typed wait, assertion, screenshot, and automatic evidence capture across close/reopen cycles and changed runtime ids. Unknown, missing, ambiguous, and cross-session resolutions produce bounded deterministic evidence; results expose both alias and resolved id across Protocol, Core, CLI, MCP, scenarios, capabilities, tests, and documentation. Debug and Release builds passed with `0` warnings/errors and all `448` tests passed in both configurations; hosted validation remains pending.
