...
- Release Commit: `37810fc0e02d0be7203eb9c50e4beeb3c723cf86` (`Release 1.4.1`)
- Scope Lock: `2026-09-03`
- Published At: `2026-09-03T06:53:44Z`
- GitHub Release: https://github.com/RolandUI/AvaScope/releases/tag/v1.4.1
...
- #111 `Release v1.4.1`; Status: `Done`.
...
- `2026-09-03`: Published v1.4.1 from exact release commit `37810fc0e02d0be7203eb9c50e4beeb3c723cf86` through successful Release workflow `33724707621`. The remote tag resolves to that commit; the public, non-prerelease GitHub Release contains all 12 expected assets; and a fresh download verified the remote 11-entry manifest by exact names, byte sizes, and SHA-256 hashes plus all three nupkg metadata versions. Nuget.org exposes Protocol, Core, and Bridge `1.4.1` as latest; fresh CDN packages pass repository-signature verification and their unsigned entries match the GitHub Release packages. Workflow logs confirm successful GitHub Packages publication for all three packages.
...
- Release State: `Released`
- Release Commit: `7363c56b9cfb692a514cac6de92b6795c964b08c` (`Release 1.4.0`)
- Scope Lock: `2026-09-02`
- Published At: `2026-09-03T02:40:24Z`
- GitHub Release: https://github.com/RolandUI/AvaScope/releases/tag/v1.4.0
...
- #103 `R1.4.0-M6 Add resilient multi-window workflows with top-level aliases`; Status: `Done`.
- #104 `R1.4.0-M7 Add conditional, retryable, and reusable workflow composition`; Status: `Done`.
- #105 `R1.4.0-M8 Add observe-act-verify and automatic runtime failure evidence packs`; Status: `Done`.
- #106 `R1.4.0-M9 Harden build-launch-attach lifecycle orchestration`; Status: `Done`.
- #107 `R1.4.0-M10 Add runtime evidence redaction, retention, and action safety policy`; Status: `Done`.
- #108 `R1.4.0-M11 Validate repeatable complex Avalonia workflows end to end`; Status: `Done`.
- #109 `Release v1.4.0`; Status: `Done`.
...
- `2026-09-03`: Published v1.4.0 from exact release commit `7363c56b9cfb692a514cac6de92b6795c964b08c` through successful Release workflow `33707881196`. The remote tag resolves to the release commit; the non-draft, non-prerelease GitHub Release contains all 12 expected assets; and a fresh download verified all 11 manifest entries by name, size, and SHA-256 plus all three package metadata versions. Nuget.org exposes Protocol, Core, and Bridge `1.4.0` as latest; all three downloaded packages pass repository-signature verification and their unsigned entries match the GitHub Release packages. The workflow logs confirm successful GitHub Packages publication for all three packages.
...
- `2026-09-02`: Completed #105 in `111e5e0` with optional observe-act-verify contracts, shared typed postcondition waits, bounded pre/post evidence, automatic partial-preserving failure packs, deterministic artifact paths, and agreeing JSON/Markdown/JUnit exports across workflow, scenario, CLI, and MCP surfaces. Coverage includes success, timeout, action failure, screenshot failure, idempotent replay without duplicate dispatch, response budgeting, schema/capability discovery, and real headless interaction. Focused coverage passed `18` tests; Debug and Release builds completed with `0` warnings/errors and all `463` tests passed in both configurations. Hosted CI `33674831016` attempt 2 passed the complete Windows, Linux, and macOS gates after an unchanged rerun cleared an unrelated transient pointer-diagnostics cleanup file lock. Started #106.
- `2026-09-02`: Completed #104 in `3247df9` with pre-dispatch compilation; typed `if`/`else`; optional leaves; bounded, idempotent `retry_until`; variables; reusable acyclic parameterized fragments; expanded validation plans; static-error aggregation; scenario preflight; deterministic nested timeline evidence; global timeout; and fixed nesting, expansion, execution, retry, artifact, variable, and fragment limits. Protocol/Core/Bridge/CLI/MCP/scenario/schema/capability/docs and response-budget coverage includes both branches, optional UI, retry success/exhaustion without duplicate side effects, cancellation, variables/fragments/cycles, all limit classes, and no-dispatch validation. Debug and Release builds completed with `0` warnings/errors and all `455` tests passed in both configurations. Hosted CI `33669183352` attempt 2 passed Windows Build/Test/Pack and artifact verification, Linux installer validation, and macOS full tests plus native runtime/preview/artifact validation after the unchanged rerun cleared a transient named-pipe harness timeout. Started #105.
- `2026-09-02`: Completed #103 in `841ddee` with workflow-scoped semantic top-level aliases that re-resolve each step, typed wait, assertion, screenshot, and automatic evidence capture across close/reopen cycles and changed runtime ids. Unknown, missing, ambiguous, and cross-session resolutions produce bounded deterministic evidence; results expose both alias and resolved id across Protocol, Core, CLI, MCP, scenarios, capabilities, tests, and documentation. Debug and Release builds passed with `0` warnings/errors and all `448` tests passed in both configurations. Hosted CI `33663497807` passed Windows Build/Test/Pack and installer/artifact verification, Linux installer validation, and macOS full tests plus native runtime/preview/artifact validation. Started #104.
...
- `2026-07-29`: The final post-hardening local v1.2.0 gate passed from `57c9ae7`. Debug and Release builds completed with `0` warnings/errors and all `417` tests passed in each configuration; three exact-version NuGet packages, two portable ZIPs, Windows/Linux installers, and the seven-entry SHA-256 manifest passed verification. Windows install/repair/doctor/MCP/uninstall, packaged doctor and real Avalonia preview, WSL Ubuntu Linux installer smoke with temporary .NET `10.0.10`, intake privacy validation, packaged version/capability checks, and NuGet/GitHub Release dry-runs passed. `v1.2.0` remains absent remotely and is ready for the exact `Release 1.2.0` commit.
