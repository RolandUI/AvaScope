...
- #108 `R1.4.0-M11 Validate repeatable complex Avalonia workflows end to end`; Status: `In Progress`.
...
- `2026-09-03`: Implemented the #108 release-shaped sample and validation harness locally. Repeat CLI and real MCP stdio runs cover two semantic window aliases, range-provider and bounds-derived custom-control drags, optional UI branches, app-defined action discovery/invocation, asynchronous typed waits, bounded retry, fragments, cross-window verification, automatic screenshots/reports, an intentional redacted failure, marker-owned retention, and exact cleanup without request coordinates, fixed sleeps, or persisted runtime ids. The failure run found and closed a policy gap in complete response-budget fallback JSON; referenced artifacts are now recursively sanitized only inside the marked run and external paths fail closed. Focused coverage passed `36` tests; full Debug and Release builds completed with `0` warnings/errors and all `503` tests passed in both configurations. Source and packaged win-x64 CLI and real MCP stdio gates each passed two success runs plus the intentional failure. Hosted cross-platform validation remains pending.
