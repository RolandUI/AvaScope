...
`audit-ui` / `audit_ui` now reports duplicate automation ids as well as missing ids. Duplicate matching follows the case-insensitive identifier search used by `find-nodes`. It returns bounded `selectorRecommendations`, preferring a unique AutomationId and then a unique name/type combination. Recommendations include their measured match count and uncertainty. Localized display text, transient runtime node ids and list positions are never suggested as stable identities.
